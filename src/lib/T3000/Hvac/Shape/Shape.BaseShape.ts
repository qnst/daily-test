...
class BaseShape extends BaseDrawingObject {
  ...
  LM_ActionRelease(event, additionalData) {
    ...
    try {
      ...
      const actionObject = GlobalData.optManager.GetObjectPtr(GlobalData.optManager.theActionStoredObjectID, false);
      ...
      if (additionalData == null) {
        ...
        if (GlobalData.optManager.theActionSVGObject == null) return;
        ...
        if (true) { // Previously Collab.AllowMessage()
          ...
          collaborationData.theRotateEndRotation = GlobalData.optManager.theRotateEndRotation;
        }
        ...
        switch (GlobalData.optManager.theActionTriggerID) {
          case ConstantData.ActionTriggerType.TABLE_ROW:
            ...
            if (tableObject) {
              const rowInfo = GlobalData.optManager.Table_GetRowAndSegment(GlobalData.optManager.theActionTriggerData);
              GlobalData.optManager.Table_SelectRowDivider(actionObject, rowInfo.row, false);
            }
          ...
          case ConstantData.ActionTriggerType.TABLE_COL:
            tableObject = actionObject.GetTable(false);
            if (tableObject) {
              const colInfo = GlobalData.optManager.Table_GetColumnAndSegment(GlobalData.optManager.theActionTriggerData);
              ...
              if (columnIndex >= 0) {
                GlobalData.optManager.Table_SelectColDivider(actionObject, columnIndex, false);
              }
          ...
          case ConstantData.ActionTriggerType.MOVEPOLYSEG:
            isTableOperation = true;
            if (true) { // Previously Collab.AllowMessage()
              const shapeObject = GlobalData.optManager.GetObjectPtr(this.BlockID, false);
          ...
          case ConstantData.ActionTriggerType.CONTAINER_ADJ:
            if (true) { // Previously Collab.AllowMessage()
              collaborationData.theActionTableLastY = GlobalData.optManager.theActionTableLastY;
            }
            break;
        }
      ...
      // Handle rotation
        ...
        GlobalData.optManager.SetObjectFrame(GlobalData.optManager.theActionStoredObjectID, actionObject.Frame);
  ...
  LM_ActionPostRelease(e) {
    var t = function () {
      if (
        GlobalData.optManager.currentModalOperation === ConstantData2.ModalOperations.FORMATPAINTER
      ) {
        if (
