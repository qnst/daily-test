...
class PolyLine extends BaseLine {
  ...
  CreateShape(svgDoc, isHidden) {
    ...
    polylineShape.SetEventBehavior(isHidden ? ConstantData2.EventBehavior.HIDDEN_OUT : ConstantData2.EventBehavior.NONE);
  ...
  GetDimensionPoints() {
    ...
    if (!this.polylist.closed && this.Dimensions & ConstantData.DimensionFlags.SED_DF_EndPts) {
      ...
      var angle = T3Gv.optManager.SD_GetCounterClockwiseAngleBetween2Points(new Point(0, 0), new Point(this.Frame.width, this.Frame.height));
    ...
    for (let segCount = 0; segCount < totalSegmentCount; segCount++) {
      ...
      if (Utils2.pointInRect(segmentRect, candidateRectPoint1) || Utils2.pointInRect(segmentRect, candidateRectPoint2)) {
        ...
        for (let subIndex = startIndex + 1; subIndex < nextIndex; subIndex++) {
          ...
          let angleRadians = T3Gv.optManager.SD_GetClockwiseAngleBetween2PointsInRadians(firstPoint, secondPoint);
  ...
  AddCorner(svgElement: any, cornerPoint: Point) {
    ...
    if (svgElement == null) {
      svgElement = T3Gv.optManager.svgObjectLayer.GetElementByID(this.BlockID);
    }
    ...
    if (svgElement != null && this.polylist.segs[cornerSegmentIndex] !== undefined) {
      ...
      hookList = T3Gv.optManager.GetHookList(linksObject, hookList, this.BlockID, this, ConstantData.ListCodes.SED_LC_MOVEHOOK, {});
      for (hookIndex = 0; hookIndex < hookList.length; hookIndex++) {
        T3Gv.objectStore.PreserveBlock(hookList[hookIndex]);
      }
      ...
      for (hookIndex = 0; hookIndex < hooksCount; hookIndex++) {
        T3Gv.optManager.SetLinkFlag(this.hooks[hookIndex].objid, ConstantData.LinkFlags.SED_L_MOVE);
      }
      ...
      if (this.Frame.x < 0 || this.Frame.y < 0) {
        ...
        T3Gv.optManager.SetObjectFrame(this.BlockID, frameRect);
      }

      // Update drawing and text objects.
      this.UpdateDrawing(svgElement);
      if (this.DataID != -1) {
        const mainElement = T3Gv.optManager.svgObjectLayer.GetElementByID(this.BlockID);
        this.LM_ResizeSVGTextObject(mainElement, this, this.Frame);
      }

      T3Gv.optManager.ShowSVGSelectionState(svgElement.GetID(), true);
      // if (Collab.AllowMessage()) {
  ...
  CreateActionTriggers(svgDocument, targetObject, additionalData, actionId) {
    ...
    let targetObj = T3Gv.optManager.GetObjectPtr(targetObject, false);
    ...
    if (this.flags & ConstantData.ObjFlags.SEDO_Lock) {
      ...
      knobProps.cursorType = ConstantData2.CursorType.DEFAULT;
    }
    ...
    if (!this.polylist.closed) {
      ...
      if (cornerKnobImages && startKnob && startKnob.SetURL) {
        startKnob.SetURL(knobProps.cursorType === ConstantData2.CursorType.NWSE_RESIZE ? cornerKnobImages.nwse : cornerKnobImages.nesw);
      }
    ...
    if (cornerKnobImages && endKnob.SetURL) {
      endKnob.SetURL(knobProps.cursorType === ConstantData2.CursorType.NWSE_RESIZE ? cornerKnobImages.nwse : cornerKnobImages.nesw);
    ...
    for (let segmentIndex = 1; segmentIndex < segmentsCount - 1; segmentIndex++) {
      ...
      if (nodeKnob) {
        ...
        if (cornerKnobImages && nodeKnob.SetURL) {
          nodeKnob.SetURL(knobProps.cursorType === ConstantData2.CursorType.NWSE_RESIZE ? cornerKnobImages.nwse : cornerKnobImages.nesw);
    ...
    if (!T3Gv.optManager.bTouchInitiated && !knobProps.locked && !this.NoGrow()) {
      knobProps.shapeType = ConstantData.CreateShapeType.OVAL;
          ...
          switch (this.polylist.segs[i].LineType) {
            ...
            case ConstantData.LineType.NURBS:
              if (i < segmentCount - 1 && this.polylist.segs[i + 1].LineType == ConstantData.LineType.NURBSSEG) {
                ...
                const docScale = T3Gv.optManager.svgDoc.docInfo.docScale;
  ...
  ModifyShape(event, x, y, actionType, segmentIndex, snapToGrid) {
    ...
    if (segmentCount !== 0) {
      ...
      T3Gv.optManager.UpdateDisplayCoordinates(dimensions, updatedPoint, ConstantData.CursorTypes.Grow, this);
  ...
  BeforeRotate(event) {
    ...
    console.log('S.PolyLine: BeforeRotate output', {
      rotateKnobCenterDivisor: T3Gv.optManager.theRotateKnobCenterDivisor,
      rotateStartRotation: T3Gv.optManager.theRotateStartRotation,
      rotateEndRotation: T3Gv.optManager.theRotateEndRotation,
      rotateStartPoint: T3Gv.optManager.theRotateStartPoint,
      rotatePivotX: T3Gv.optManager.theRotatePivotX,
      rotatePivotY: T3Gv.optManager.theRotatePivotY
    });
  }
  ...
  AdjustRotate(mouseX, mouseY, event) {
    ...
    const startX = T3Gv.optManager.theRotateStartPoint.x;
    const startY = T3Gv.optManager.theRotateStartPoint.y;
    ...
    if (T3Gv.docUtil.docConfig.enableSnap && !overrideSnaps) {
      rotationAngle = Math.round(rotationAngle / T3Gv.optManager.theRotateSnap) * T3Gv.optManager.theRotateSnap;
    }
    ...
    if (boundingRect.x >= 0 && boundingRect.y >= 0) {
      ...
      T3Gv.optManager.theRotateEndRotation = rotationAngle;
      this.Rotate(T3Gv.optManager.theActionSVGObject, rotationAngle);
    }
  ...
  Rotate(svgElement, rotationAngle) {
    ...
    svgElement.SetRotation(rotationAngle, T3Gv.optManager.theRotatePivotX, T3Gv.optManager.theRotatePivotY);
  ...
  AfterRotateShape(event, actionType) {
    ...
    const endRotation = T3Gv.optManager.theRotateEndRotation;
    const pivotPoint = {
      x: T3Gv.optManager.theRotatePivotX,
      y: T3Gv.optManager.theRotatePivotY
    };
    const rotationAngle = -(endRotation - T3Gv.optManager.theRotateStartRotation) / (180 / ConstantData.Geometry.PI);

    T3Gv.optManager.RotatePointAroundPoint(pivotPoint, T3Gv.optManager.theRotateStartPoint, rotationAngle);
    ...
    if (actionType !== ConstantData.ActionTriggerType.MOVEPOLYSEG) {
      ...
      if (T3Gv.optManager.ob && T3Gv.optManager.ob.Frame) {
        T3Gv.optManager.MaintainLink(event, this, T3Gv.optManager.ob, ConstantData.ActionTriggerType.ROTATE);
        T3Gv.optManager.ob = {};
      }
      ...
      T3Gv.optManager.SetLinkFlag(event, ConstantData.LinkFlags.SED_L_MOVE);
  ...
  AdjustLine(event, newX, newY, actionType, snapToGrid) {
    ...
    const dimensions = this.GetDimensionsForDisplay();
    T3Gv.optManager.UpdateDisplayCoordinates(dimensions, newPoint, ConstantData.CursorTypes.Grow, this);
  ...
  PolyGetTargetPointList(hookFlags) {
    ...
    if (this.polylist && this.polylist.closed && this.StyleRecord.Line.BThick && !(hookFlags & ConstantData.HookFlags.SED_LC_ShapeOnLine)) {
      let outerInflatedPoints = T3Gv.optManager.InflateLine(polyPoints, this.StyleRecord.Line.BThick, true, true);
      let innerInflatedPoints = T3Gv.optManager.InflateLine(polyPoints, this.StyleRecord.Line.BThick, true, false);
  ...
  GetTargetPoints(hookPoint, hookFlags, targetObjectID) {
    ...
    if (targetObjectID != null && targetObjectID >= 0 && T3Gv.optManager.GetObjectPtr(targetObjectID, false).DrawingObjectBaseClass === ConstantData.DrawingObjectBaseClass.SHAPE) {
      switch (hookPoint.id) {
  ...
  BaseShape_PolyGetTargets(hookPoint, hookFlags, targetFrame) {
    ...
    const enableSnap = T3Gv.docUtil.docConfig.enableSnap && !(hookFlags & ConstantData.HookFlags.SED_LC_NoSnaps);
    if (enableSnap) {
  ...
  GetPerimeterPoints(event, hookPoints, hookPointID, rotationAngle, scaleFactor, targetObjectID) {
    ...
    let targetObject = T3Gv.optManager.GetObjectPtr(targetObjectID, false);
  ...
  MaintainPoint(point, targetID, targetObject, drawingObject, actionType) {
    ...
    if (this.polylist && this.polylist.closed && this.StyleRecord.Line.BThick && drawingObject instanceof BaseLine) {
      ...
      if (!isHit || hitInfo.lpHit < 0) {
        let inflatedTargetPolyPoints = T3Gv.optManager.InflateLine(targetPolyPoints, targetObject.StyleRecord.Line.BThick, true, false);
        isHit = Utils3.LineDStyleHit(inflatedTargetPolyPoints, { x: point.x, y: point.y }, 2, -12, hitInfo);
        if (isHit && hitInfo.lpHit >= 0) {
          polyPoints = T3Gv.optManager.InflateLine(polyPoints, this.StyleRecord.Line.BThick, true, false);
        } else {
          let fullyInflatedTargetPolyPoints = T3Gv.optManager.InflateLine(targetPolyPoints, targetObject.StyleRecord.Line.BThick, true, true);
          isHit = Utils3.LineDStyleHit(fullyInflatedTargetPolyPoints, { x: point.x, y: point.y }, 2, -12, hitInfo);
          if (isHit && hitInfo.lpHit >= 0) {
            polyPoints = T3Gv.optManager.InflateLine(polyPoints, this.StyleRecord.Line.BThick, true, true);
          }
        }
      }
    }
    ...
    if (segmentIndex >= 0 && hitSegmentIndex >= 0) {
      ...
      if (drawingObject.DrawingObjectBaseClass === ConstantData.DrawingObjectBaseClass.SHAPE) {
        return T3Gv.optManager.LineCheckPoint(tempObject, point) || T3Gv.optManager.Lines_MaintainDist(tempObject, tempHitResult, actionType, point), true;
      }

      switch (actionType) {
        case ConstantData.ActionTriggerType.POLYLNODE:
          actionType = (T3Gv.optManager.theActionTriggerData === segmentIndex - 1 || T3Gv.optManager.theActionTriggerData === segmentIndex) &&
            tempObject.StartPoint.x === polyPoints[T3Gv.optManager.theActionTriggerData].x &&
            tempObject.StartPoint.y === polyPoints[T3Gv.optManager.theActionTriggerData].y ?
        ...
        case ConstantData.ActionTriggerType.ROTATE:
          return T3Gv.optManager.Lines_MaintainDist(tempObject, tempHitResult, actionType, point), true;
        default:
          actionType = ConstantData.ActionTriggerType.LINEEND;
      }

      if (T3Gv.optManager.LineCheckPoint(tempObject, point)) return true;

      if (targetObject.polylist.segs.length === this.polylist.segs.length) {
        T3Gv.optManager.Lines_MaintainDistWithinSegment(this, targetObject, segmentIndex, point);
        return true;
      } else {
        return T3Gv.optManager.Lines_Intersect(tempObject, drawingObject, point) || T3Gv.optManager.Lines_MaintainDist(tempObject, tempHitResult, actionType, point), true;
      }
    }
  ...
  ChangeTarget(
    ...
    targetObject = T3Gv.optManager.GetObjectPtr(objectTargetID, false);
    if (
  ...
  LM_DrawRelease(event) {
    ...
    //   messagePayload.attributes.StyleRecord = Utils1.DeepCopy(T3Gv.optManager.theDrawShape.StyleRecord);
    //   messagePayload.attributes.StartArrowID = T3Gv.optManager.theDrawShape.StartArrowID;
    //   messagePayload.attributes.EndArrowID = T3Gv.optManager.theDrawShape.EndArrowID;
    //   messagePayload.attributes.StartArrowDisp = T3Gv.optManager.theDrawShape.StartArrowDisp;
    //   messagePayload.attributes.ArrowSizeIndex = T3Gv.optManager.theDrawShape.ArrowSizeIndex;
    //   messagePayload.attributes.TextGrow = T3Gv.optManager.theDrawShape.TextGrow;
    //   messagePayload.attributes.TextAlign = T3Gv.optManager.theDrawShape.TextAlign;
    //   messagePayload.attributes.TextDirection = T3Gv.optManager.theDrawShape.TextDirection;
    //   messagePayload.attributes.Dimensions = T3Gv.optManager.theDrawShape.Dimensions;
    //   messagePayload.attributes.StartPoint = Utils1.DeepCopy(T3Gv.optManager.theDrawShape.StartPoint);
    //   messagePayload.attributes.EndPoint = Utils1.DeepCopy(T3Gv.optManager.theDrawShape.EndPoint);
    //   messagePayload.attributes.Frame = Utils1.DeepCopy(T3Gv.optManager.theDrawShape.Frame);
    ...
    //     messagePayload.CreateList = [T3Gv.optManager.theDrawShape.BlockID];
    //   }
  ...
  LM_DrawExtend(event: any) {
    ...
    if (
      ...
      let documentCoordinates = T3Gv.optManager.svgDoc.ConvertWindowToDocCoords(
  ...
  RightClick(event) {
    ...
    let svgElement = T3Gv.optManager.svgObjectLayer.FindElementByDOMElement(event.currentTarget);
    ...
    let targetObject = T3Gv.optManager.GetObjectPtr(elementId, false);
    let spellIndex = -1;
    ...
    if (hitDetails.hitcode) {
      T3Gv.optManager.RightClickParams.segment = hitDetails.segment;
    }

    // Check if there is an active text edit.
    if (T3Gv.optManager.GetActiveTextEdit() !== null) {
      let activeEdit = T3Gv.optManager.svgDoc.GetActiveEdit();
      spellIndex = -1;
      if (activeEdit) {
  ...
  WriteSDFAttributes(writer, options, usePolyListDimensions) {
    ...
    if (
      ...
      polyPoints = T3Gv.optManager.InflateLine(tempArray, polyObj.StyleRecord.Line.BThick, true, false);
  ...
  Pr_PolyLGetEllipseParam(event, segmentIndex, svgDocument) {
    ...
    if (Math.sqrt(deltaX * deltaX + deltaY * deltaY) >= 1 && Math.abs(deltaY) > 1) {
      ...
      if (!ellipseAxesElement) {
        ellipseAxesElement = T3Gv.optManager.svgDoc.CreateShape(ConstantData.CreateShapeType.POLYLINE);
  ...
  GetAreaWidthAndHeightText(polyPoints) {
    ...
    if (this.polylist && this.polylist.closed && T3Gv.optManager.IsRectangularPolygon(polyPoints)) {
      angle = T3Gv.optManager.SD_GetCounterClockwiseAngleBetween2Points(polyPoints[0], polyPoints[1]);
  ...
  SetSegmentAngle(event, segmentIndex, angle) {
    ...
    rotationAngle = T3Gv.optManager.SD_GetCounterClockwiseAngleBetween2Points(segmentPoints[0], segmentPoints[1]);
    ...
    if (this.Dimensions & ConstantData.DimensionFlags.SED_DF_InteriorAngles) {
      ...
      counterClockwiseAngle = T3Gv.optManager.SD_GetCounterClockwiseAngleBetween2Points(previousSegmentPoints[0], previousSegmentPoints[1]);
  ...
  CancelObjectDraw() {
    ...
    T3Gv.optManager.AddToDirtyList(this.BlockID);
    T3Gv.optManager.RenderDirtySVGObjects();
    T3Gv.optManager.SelectObjects([this.BlockID], false, true);
  ...
  GetPolyRectangularInfo(dimensionInfo: any): boolean {
    ...
    for (let index = 0; index < polyPoints.length - 1 && isRectangle; index++) {
      let currentAngle = T3Gv.optManager.SD_GetCounterClockwiseAngleBetween2Points(polyPoints[index], polyPoints[index + 1]);
      if (index > 0) {
    ...
    let firstSideAngle = T3Gv.optManager.SD_GetCounterClockwiseAngleBetween2Points(polyPoints[0], polyPoints[1]);
    ...
    if (!hasNegativeCoordinates && (newUnrotatedBBox.x < 0 || newUnrotatedBBox.y < 0)) {
      if (offsetEnabled) {
        ...
        for (let idx = 0; idx < enclosedObjects.length; idx++) {
          ...
          T3Gv.optManager.SetLinkFlag(enclosedObjects[idx], ConstantData.LinkFlags.SED_L_MOVE);
