...
class BaseDrawingObject {
  ...
  AfterModifyShape(shape: any, additionalData: any): void {
    ...
    T3Gv.optManager.SetLinkFlag(shape, ConstantData.LinkFlags.SED_L_MOVE);
  ...
  UpdateDimensionsFromTextForHookedObject(textObject: any, text: string, hookedObjectInfo: any): void {
    ...
    if (targetPoints && isPointInRect) {
      ...
      T3Gv.optManager.SetLinkFlag(hookedObject.BlockID, ConstantData.LinkFlags.SED_L_MOVE);
  ...
  DrawDimensionAngle(container: any, pathCreator: any, segmentIndex: number, dimensionPoints: Point[]): void {
    ...
    if (dimensionInfo) {
      ...
      dimensionInfo.text.SetConstraints(T3Gv.optManager.theContentHeader.MaxWorkDim.x, 0, 0);
  ...
  GetHookedObjectDescList(dimensionPoints: Point[], context: any): any[] {
    ...
    let result: any[] = [];
    let linkManager = T3Gv.optManager.GetObjectPtr(T3Gv.optManager.theLinksBlockID, false);
  ...
  GetDimensionTextInfo1(
    ...
    if (
      ...
      const linkObj = T3Gv.optManager.GetObjectPtr(T3Gv.optManager.theLinksBlockID, false);
  ...
  MoveBehindAllLinked() {
    ...
    let linksObj = T3Gv.optManager.GetObjectPtr(T3Gv.optManager.theLinksBlockID, true);
    if (linksObj) {
