...
class OptHandler {
  ...
  SD_GetVisioTextChild(objectId) {
    ...
    const parentObject = GlobalData.optManager.GetObjectPtr(objectId);

    if (parentObject && parentObject.TextFlags & ConstantData.TextFlags.SED_TF_AttachD) {
      const associatedObject = GlobalData.optManager.GetObjectPtr(parentObject.associd);
        ...
        switch (parentObjects[objectIndex].objecttype) {
          case ConstantData.ObjectTypes.SD_OBJT_GANTT_CHART:
            if (layersManager.layers[layersManager.activelayer].zList.indexOf(parentObjects[objectIndex].BlockID) >= 0) {
              ...
              GlobalData.optManager.GanttFormat(parentObjects[objectIndex].BlockID, true, false, true, null);
            }
            break;
  ...
  CN_ChangeHook(sourceObject, hookIndex, isAdding, hookData) {
    ...
    if (isAdding) {
      ...
      // Special handling for Gantt chart connectors
        ...
        endPoint.x = GlobalData.optManager.GetDependencyLineEndX(sourceObject);
        ...
        for (tempCounter = 0; tempCounter < totalSelected; tempCounter++) {
          ...
          if (!(shape instanceof PolyLineContainer)) {
            ...
            if (shape instanceof BaseLine) {
              ...
              let textChildId = GlobalData.optManager.SD_GetVisioTextChild(selectedList[tempCounter]);
              if (textChildId >= 0) {
                textChild = GlobalData.optManager.GetObjectPtr(textChildId, true);
                if (textChild) {
  ...
  UngroupShape(groupId, maintainLinksFlag) {
    ...
    const textChildId = GlobalData.optManager.SD_GetVisioTextChild(groupId);
    ...
    if (textChildId >= 0) {
      existingObject = GlobalData.optManager.GetObjectPtr(textChildId);
  ...
  GetSelectionContext(): any {
    ...
    if (targetObjectId !== -1) {
      ...
      if (businessManager && !GlobalData.optManager.Comment_IsTarget(targetObjectId)) {
        selectionContexts.push(Resources.Contexts.Automation);
      }
