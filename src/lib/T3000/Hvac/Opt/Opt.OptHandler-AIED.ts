...
class OptHandler {
  ...
  GetFractionDenominator() {
    ...
    let denominator;
    const rulerScale = T3Gv.docHandler.rulerSettings.majorScale;
  ...
  UpdateSelectionAttributes(selectedObjects) {
    ...
    for (objectIndex = 0; objectIndex < objectCount; objectIndex++) {
      ...
      objectVisioTextChild = -1; // T3Gv.optManager.SD_GetVisioTextChild(objectId);
    ...
    if (T3Gv.docHandler.rulerSettings.showpixels && selectionAttributes.fontsize >= 0) {
  ...
  UpdateDisplayCoordinates(dimensions, position, cursorType, drawingObject) {
    ...
    if (T3Gv.docHandler.documentConfig.showRulers) {
      ...
      if (dimensions) {
        const xLength = this.GetLengthInRulerUnits(dimensions.x, false, T3Gv.docHandler.rulerSettings.originx, showFractionalInches);
        const yLength = this.GetLengthInRulerUnits(dimensions.y, false, T3Gv.docHandler.rulerSettings.originy, showFractionalInches);
  ...
  FindConnect(targetObjectId, drawingObject, hookPoints, showVisuals, isAttachMode, allowJoin, eventPosition) {
    ...
    for (let pointIndex = 0; pointIndex < hookPoints.length; pointIndex++) {
      ...
      if (hitResult && hitResult.hitcode) {
        const targetObject = this.GetObjectPtr(hitResult.objectid, false);
  ...
  Undo(restoreSequence: boolean, cancelModalOperation: boolean): boolean {
    ...
    if (T3Gv.docHandler.PagesNotEqual(sessionObject.Page, T3Gv.optManager.theContentHeader.Page)) {
  ...
  UpdateLinks() {
    ...
    const sessionData = this.GetObjectPtr(this.theSEDSessionBlockID, false);
    const originalSnapEnabled = T3Gv.docHandler.documentConfig.enableSnap;

    // Disable snapping during link updates
    T3Gv.docHandler.documentConfig.enableSnap = false;
    ...
    T3Gv.docHandler.documentConfig.enableSnap = originalSnapEnabled;
  ...
  LM_SetupMove(event) {
    ...
    // Check for Gantt bar special case
    if (
      drawingObject.objecttype === ConstantData.ObjectTypes.SD_OBJT_GANTT_BAR &&
      (drawingObject.datasetElemID >= 0 && this.GanttGetParentID(drawingObject.datasetElemID) < 0)
    ) {
      console.log("O.Opt LM_SetupMove - Output: false (Special Gantt bar)");
      return false;
    }
    ...
    // Handle Gantt bar specially
    let objectsToMove = [];
    if (drawingObject.objecttype === ConstantData.ObjectTypes.SD_OBJT_GANTT_BAR) {
      this.theMoveBounds = drawingObject.GetMoveRect(false, false);
      objectsToMove.push(this.dragTargetId);
      this.PinRect = {};
      this.PinRect = drawingObject.AdjustPinRect(this.PinRect, true);
    } else {
    ...
    for (let i = 0; i < objectCount; ++i) {
      ...
      if (T3Gv.docHandler.documentConfig.enableSnap &&
  ...
  AllowSnapToShapes() {
    ...
    const result = T3Gv.docHandler.documentConfig.snapToShapes;
  ...
  LM_MovePostRelease(completeOperation, moveData) {
    ...
    if (completeOperation) {
      ...
      // Handle object connections
        ...
        const businessManager = Business.GetSelectionBusinessManager(this.dragTargetId);
      ...
      // Handle Gantt bar adjustments
      if (this.IsGanttBar(this.dragTargetId)) {
  ...
  DragDuplicate(event) {
    ...
    if (isCtrlKeyPressed) {
      ...
      if (targetObject) {
        // Certain object types don't support duplication
        switch (targetObject.objecttype) {
          case objectTypes.SD_OBJT_MINDMAP_MAIN:
          case objectTypes.SD_OBJT_GANTT_CHART:
          case objectTypes.SD_OBJT_GANTT_BAR:
  ...
  LM_MoveDuringTrack(position) {
    ...
    if (this.LinkParams && this.LinkParams.AutoHeal) {
      ...
      if (horizontalDistance > 50 ||
        ...
        let indexInDirtyList = this.theDirtyList.indexOf(this.dragTargetId);
        ...
        this.GetMoveList(this.dragTargetId, true, true, false, this.theMoveBounds, false);
      }
    }
  ...
  HandleObjectDragMoveCommon(mouseX, mouseY, skipScrolling, event) {
    ...
    const constrainMovementToBounds = () => {
      if (T3Gv.optManager.dragDeltaX < -T3Gv.optManager.theMoveBounds.x) {
        T3Gv.optManager.dragDeltaX = -T3Gv.optManager.theMoveBounds.x;
      }
      if (T3Gv.optManager.dragDeltaY < -T3Gv.optManager.theMoveBounds.y) {
    ...
    if (isSnapDisabled && this.Dynamic_Guides) {
      this.DynamicSnapsRemoveGuides(this.Dynamic_Guides);
      this.Dynamic_Guides = null;
    }
    ...
    if (!isSnapDisabled && this.AllowSnapToShapes()) {
      ...
      const snapOptions = {};
      const snapTargetId = targetObject.CanSnapToShapes(snapOptions);
    ...
    if (T3Gv.docHandler.documentConfig.enableSnap && !isSnapDisabled) {
      objectRect = targetObject.GetSnapRect();
      targetRect = $.extend(true, {}, objectRect);
      targetRect.x += T3Gv.optManager.dragDeltaX;
      targetRect.y += T3Gv.optManager.dragDeltaY;
      ...
      // Use center snapping if enabled
      else if (T3Gv.docHandler.documentConfig.centerSnap) {
        ...
        mousePosition = T3Gv.docHandler.SnapToGrid(mousePosition);
      ...
      // Use regular rect snapping
        ...
        const snapResult = T3Gv.docHandler.SnapRect(targetRect);
  ...
  /**
   * Checks if the specified object is a Gantt bar
   * @param objectId - ID of the object to check
   * @returns True if object is a Gantt bar, false otherwise
   */
  IsGanttBar(objectId) {
    console.log("O.Opt IsGanttBar - Input:", objectId);

    const object = this.GetObjectPtr(objectId);
    const isGanttBar = !!object &&
      object.objecttype === ConstantData.ObjectTypes.SD_OBJT_GANTT_BAR;

    console.log("O.Opt IsGanttBar - Output:", isGanttBar);
    return isGanttBar;
  }
  ...
  ResizeSVGDocument() {
    ...
    T3Gv.docHandler.ResizeDocument(sessionData.dim.x, sessionData.dim.y);
  ...
  LM_ShapeIconClick(event, objectId, iconType, userData) {
    ...
    switch (iconType) {
      case ConstantData.ShapeIconType.TRELLOLINK:
        // Handle Trello card link
        let dataRecordId, trelloCardUrl;
        const noteCell = drawingObject.IsNoteCell(userData);

        // Get data record ID from cell or object
        dataRecordId = noteCell ? noteCell.datarecordID : drawingObject.datasetElemID;

        // If we have a valid record, get the Trello URL
        if (dataRecordId >= 0) {
          trelloCardUrl = ListManager.SDData.GetValue(
            dataRecordId,
            ListManager.GanttFieldNameList[ListManager.GanttTaskFields.TASK_TRELLO_CARD_URL]
          );
        }

        // Follow the URL if it exists
        if (trelloCardUrl && trelloCardUrl.length) {
          if (event.gesture) {
            event.gesture.stopDetect();
          }
          SDUI.Commands.MainController.Hyperlinks.FollowHyperlink(trelloCardUrl);
          console.log("O.Opt LM_ShapeIconClick - Output: true (Followed Trello link)");
          return true;
        }
        console.log("O.Opt LM_ShapeIconClick - Output: false (No Trello link)");
        return false;
      ...
      case ConstantData.ShapeIconType.HYPERLINK:
        ...
        if (hyperlinkUrl !== '') {
          ...
          SDUI.Commands.MainController.Hyperlinks.FollowHyperlink(hyperlinkUrl);
          console.log("O.Opt LM_ShapeIconClick - Output: true (Followed hyperlink)");
          return true;
        }
        console.log("O.Opt LM_ShapeIconClick - Output: true (No hyperlink content)");
        return true;
      ...
      case ConstantData.ShapeIconType.EXPANDTABLE:
        ...
        this.SD_GanttExpandContract(drawingObject.BlockID, cellForExpand, true, userData);
      ...
      case ConstantData.ShapeIconType.COLLAPSETABLE:
        ...
        this.SD_GanttExpandContract(drawingObject.BlockID, cellForCollapse, false, userData);
  ...
  ActivateTextEdit(drawingElement, event, preventSelectionChange, textData) {
    ...
    // Check for Visio text child
    let visioTextChildId = -1;
    if (visioTextChildId >= 0) {
      objectId = visioTextChildId;
    }
    ...
    try {
      ...
      if (!isTextOnlyObject) {
        ...
        if (T3Gv.docHandler.documentConfig.enableSnap &&
          ...
          docCoordinates = T3Gv.docHandler.SnapToGrid(docCoordinates);
        }
      }
      ...
      if (T3Gv.gBusinessManager instanceof FloorPlan) {
  ...
  DragDropObjectDone(event, additionalData) {
    ...
    try {
      ...
      if (!isTextOnlyObject) {
        ...
        if (T3Gv.docHandler.documentConfig.enableSnap && !isConnectOperation && !isSnappingOverridden) {
          docCoordinates = T3Gv.docHandler.SnapToGrid(docCoordinates);
        }
      }
  ...
  StampObjectMoveCommon(mouseX, mouseY, event) {
    ...
    // Handle grid snapping if enabled
    if (T3Gv.docHandler.documentConfig.enableSnap && !isSnapDisabled && !isTextOnlyObject) {
      let objectRect, targetRect;

      // Get appropriate rectangles based on whether we're moving multiple objects
      if (this.theMoveList && this.theMoveList.length) {
        objectIndex = this.theMoveList.indexOf(this.actionStoredObjectId);
      ...
      if (dragEnclosingRect && targetRect) {
        ...
        // Apply center snap if enabled
          ...
          const snapResult = T3Gv.docHandler.SnapToGrid(currentPosition);
          if (snapOffset.x === null) {
            currentPosition.x = snapResult.x;
          }
          if (snapOffset.y === null) {
        ...
        // Apply standard grid snapping
          ...
          const snapResult = T3Gv.docHandler.SnapRect(snapRect);
          if (snapOffset.x === null) {
            currentPosition.x += snapResult.x;
          }
          if (snapOffset.y === null) {
  ...
  /**
   * Gets the text child of a Visio shape
   * @param objectId - ID of the parent object
   * @returns ID of the text child or -1 if not found
   */
  SD_GetVisioTextChild(objectId) {
    console.log("O.Opt SD_GetVisioTextChild - Input:", objectId);

    const parentObject = T3Gv.optManager.GetObjectPtr(objectId);

    if (parentObject && parentObject.TextFlags & ConstantData.TextFlags.SED_TF_AttachD) {
      const associatedObject = T3Gv.optManager.GetObjectPtr(parentObject.associd);

      if (associatedObject &&
        associatedObject.moreflags & ConstantData.ObjMoreFlags.SED_MF_VisioText) {

        console.log("O.Opt SD_GetVisioTextChild - Output: Found Visio text child", parentObject.associd);
        return parentObject.associd;
      }
    }

    console.log("O.Opt SD_GetVisioTextChild - Output: No Visio text child found (-1)");
    return -1;
  }
      ...
      for (objectIndex = 0; objectIndex < objectCount; objectIndex++) {
        switch (parentObjects[objectIndex].objecttype) {
          case ConstantData.ObjectTypes.SD_OBJT_GANTT_CHART:
            if (layersManager.layers[layersManager.activelayer].zList.indexOf(parentObjects[objectIndex].BlockID) >= 0) {
              T3Gv.optManager.UpdateLinks();
              T3Gv.optManager.GanttFormat(parentObjects[objectIndex].BlockID, true, false, true, null);
            }
            break;
  ...
  CN_ChangeHook(sourceObject, hookIndex, isAdding, hookData) {
    ...
    if (isAdding) {
      ...
      // Special handling for Gantt chart connectors
      else if (
        sourceObject instanceof SegmentedLine &&
        sourceObject.objecttype === ConstantData.ObjectTypes.SD_OBJT_GANTT_CONNECTOR &&
        sourceObject.hooks.length >= 2
      ) {
        const endPoint = new Point(0, 0);
        endPoint.x = T3Gv.optManager.GetDependencyLineEndX(sourceObject);
        endPoint.y = sourceObject.EndPoint.y;

        // Update the SVG element if it exists
        const svgElement = this.svgObjectLayer.GetElementByID(sourceObject.BlockID);
        if (svgElement) {
          sourceObject.EndPoint.x = endPoint.x;
          sourceObject.EndPoint.y = endPoint.y;
          sourceObject.ModifyShape(
            svgElement,
            endPoint.x,
            endPoint.y,
        ...
        for (tempCounter = 0; tempCounter < totalSelected; tempCounter++) {
          ...
          if (!(shape instanceof PolyLineContainer)) {
            ...
            if (shape instanceof BaseLine) {
              ...
              // For non-baseline objects, update rotation and optionally adjust Visio text child
              shape.RotationAngle = angleDegrees;
              shape.UpdateFrame(shape.Frame);
              let textChildId = T3Gv.optManager.SD_GetVisioTextChild(selectedList[tempCounter]);
              if (textChildId >= 0) {
                textChild = T3Gv.optManager.GetObjectPtr(textChildId, true);
                if (textChild) {
                  if (textChild.VisioRotationDiff) {
                    textChild.RotationAngle = angleDegrees - textChild.VisioRotationDiff;
                  } else {
                    textChild.RotationAngle = angleDegrees;
                  }
                  textChild.UpdateFrame(textChild.Frame);
                  this.AddToDirtyList(textChildId);
                }
              }
              if (this.theContentHeader.flags & ConstantData.ContentHeaderFlags.CT_DA_NoAuto) {
  ...
  AddtoDelete(objectIds: number[], isForced: boolean, additionalData: any) {
    ...
    for (currentIndex = 0; currentIndex < objectCount; currentIndex++) {
      ...
      if (currentObj != null) {
        ...
        if (currentObj.objecttype === ConstantData.ObjectTypes.SD_OBJT_GANTT_CHART) {
          this.GanttAddtoDelete(objectIds, isForced, currentObj);
        } else if (currentObj.objecttype === ConstantData.ObjectTypes.SD_OBJT_TABLE_WITH_SHAPECONTAINER) {
          ...
          if (!flagSkip) {
            ...
            if (currentObj.associd >= 0) {
              ...
              if (assocObj) {
                ...
                if (assocObj.moreflags & ConstantData.ObjMoreFlags.SED_MF_VisioText) {
                  assocFlag = true;
                }
  ...
  IsPlanningDocument() {
    console.log("O.Opt IsPlanningDocument - Input: no parameters");

    const layersManager = T3Gv.optManager.GetObjectPtr(this.theLayersManagerBlockID, false);
    const numberOfLayers = layersManager.nlayers;
    let planningDocumentType = 0;
    const layers = layersManager.layers;

    for (let index = 0; index < numberOfLayers; ++index) {
      switch (layers[index].layertype) {
        case ConstantData.LayerTypes.SD_LAYERT_MINDMAP:
          if (index === layersManager.activelayer) {
            planningDocumentType = ConstantData.LayerTypes.SD_LAYERT_MINDMAP;
          }
          break;
        case ConstantData.LayerTypes.SD_LAYERT_GANTT:
          if (index === layersManager.activelayer) {
            planningDocumentType = ConstantData.LayerTypes.SD_LAYERT_GANTT;
          }
          break;
      }
    }

    console.log("O.Opt IsPlanningDocument - Output:", planningDocumentType);
    return planningDocumentType;
  }
  ...
  Redo(shouldCancelModalOperation) {
    ...
    const currentRulerSettings = T3Gv.docHandler.rulerSettings;
    if (T3Gv.docHandler.RulersNotEqual(updatedSessionData.rulerSettings, currentRulerSettings)) {
      T3Gv.docHandler.SetRulers(updatedSessionData.rulerSettings, true);
    }

    // Update page settings if changed
    if (T3Gv.docHandler.PagesNotEqual(updatedSessionData.Page, T3Gv.optManager.theContentHeader.Page)) {
    ...
    if (previousLayerType != newLayerType) {
      switch (newLayerType) {
        case ConstantData.LayerTypes.SD_LAYERT_MINDMAP:
  ...
  PasteObjects(event) {
    ...
    try {
      ...
      const textEditSession = this.GetObjectPtr(this.theTEDSessionBlockID, false);
      const activeTableId = this.Table_GetActiveID();
      let isGanttTable = false;
      let shouldPasteInGantt = false;

      // Check if the active table is a Gantt chart
      if (activeTableId >= 0) {
        tableObject = this.GetObjectPtr(activeTableId, false);

        if (tableObject && tableObject.objecttype === ConstantData.ObjectTypes.SD_OBJT_GANTT_CHART) {
          isGanttTable = true;

          // Check if we have LM clipboard content that should be pasted into Gantt
          if (this.theContentHeader.ClipboardBuffer &&
            this.theContentHeader.ClipboardType === ConstantData.ClipboardType.LM) {
            shouldPasteInGantt = true;
          }
        }
      }

      // Case 1: We're in text editing mode and not pasting into Gantt
      if ((textEditSession.theActiveTextEditObjectID !== -1 || this.bInNoteEdit) && !shouldPasteInGantt) {
  ...
  VirtualKeyboardLifter(element: any, isActive: boolean) {
    ...
    if (isActive) {
      ...
      if (frameChanged || forceUpdate) {
        ...
        let windowCoords = T3Gv.docHandler.DocObject().ConvertDocToWindowCoords(elementFrame.x, elementFrame.y);

        // Convert width and height from document lengths to window lengths.
        let windowWidth = T3Gv.docHandler.DocObject().ConvertDocToWindowLength(elementFrame.width);
        if (windowWidth === 0) {
          windowWidth = 1;
        }
        let windowHeight = T3Gv.docHandler.DocObject().ConvertDocToWindowLength(elementFrame.height);
        if (windowHeight === 0) {
          windowHeight = 1;
        }
  ...
  FontSizeToPoints(fontSizeValue: number): number {
    ...
    const { rulerSettings, svgDoc } = T3Gv.docHandler;
    ...
    for (let index = 0; index < totalIds; index++) {
      ...
      if (!skipContainerParents || !Business.HasContainerParent(currentObject)) {
        ...
        switch (currentObject.objecttype) {
          case objectTypes.SD_OBJT_GANTT_CHART:
          case objectTypes.SD_OBJT_GANTT_BAR:
          case objectTypes.SD_OBJT_GANTT_CONNECTOR:
            continue;
          // For swimlane grid-related types, no extra action in switch required.
  ...
  SetBackgroundColor(): void {
    ...
    const sessionObject = T3Gv.optManager.GetObjectPtr(T3Gv.optManager.theSEDSessionBlockID, false);
    const backgroundElement = T3Gv.docHandler.GetBackground();
  ...
  MakeSameSize(sizeOption) {
    ...
    for (let index = 0; index < selectedCount; ++index) {
      ...
      if ((T3Gv.docHandler.documentConfig.centerSnap && T3Gv.docHandler.documentConfig.enableSnap) ||
        T3Gv.docHandler.documentConfig.enableSnap === false) {
  ...
  StampTextObjectOnTapDone(event, optionalParam) {
    ...
    if (!isTextOnly) {
      ...
      if (T3Gv.docHandler.documentConfig.enableSnap && !hasValidConnection) {
        docCoords = T3Gv.docHandler.SnapToGrid(docCoords);
      }
    }
        ...
        if (selectedCell.flags & ListManager.Table.CellFlags.SDT_F_Clickhere) {
          switch (selectedCell.celltype) {
            ...
            case ListManager.Table.CellTypes.SDT_CT_GANTTTASK:
              replaceIndex = Resources.ReplaceTextStrings.Indexes.TaskClick;
              break;
            default:
  ...
  UngroupShape(groupId, maintainLinksFlag) {
    ...
    const textChildId = T3Gv.optManager.SD_GetVisioTextChild(groupId);
    ...
    // Handle text child if present
    if (textChildId >= 0) {
      existingObject = T3Gv.optManager.GetObjectPtr(textChildId);
      existingObject.moreflags = Utils2.SetFlag(
        existingObject.moreflags,
        ConstantData.ObjMoreFlags.SED_MF_VisioText,
        false
      );
      existingObject.associd = -1;
      this.BringObjectToFrontofLayer(textChildId);
    }
  ...
  GetPastePosition(): { x: number; y: number } {
    ...
    const offset = 100;
    const workArea = T3Gv.docHandler.svgDoc.GetWorkArea();
    let scale = T3Gv.docHandler.svgDoc.docInfo.docToScreenScale;
  ...
  GetSelectionContext(): any {
    ...
    if (targetObjectId !== -1) {
      ...
      if (businessManager && !T3Gv.optManager.Comment_IsTarget(targetObjectId)) {
        selectionContexts.push(Resources.Contexts.Automation);
      }
  ...
  GeneratePreviewPNG(callback, maxWidth, maxHeight, options) {
    ...
    const exportFormat = options && options.format || Resources.SDJSExportType.PNG;

    if (exportFormat == Resources.SDJSExportType.PNG || exportFormat == Resources.SDJSExportType.JPEG) {
      ...
      this.GenerateEncapsulatedSVG((svgData, success) => {
        if (svgData && success) {
          ...
          image.onload = function () {
            ...
            if (exportFormat == Resources.SDJSExportType.JPEG || (options && options.fillBackground)) {
            ...
            const mimeType = exportFormat == Resources.SDJSExportType.JPEG ? 'image/jpeg' : 'image/png';
  ...
  Table_UpdateSelectionAttributes(tableId, skipUpdate) {
    ...
    if (tableData && tableData.select >= 0) {
      ...
      switch (selectedCell.celltype) {
        ...
        case cellTypes.SDT_CT_GANTT_DATEBLOCK_TITLE:
          this.SelectionState.IsTargetTable = false;
          this.SelectionState.ntablesselected = 0;
          this.SelectionState.NTableRows = GlobalDatagOptions.newTableRows;
          this.SelectionState.NTableCols = GlobalDatagOptions.newTableCols;
   ...
   * @param shape - The drawing object that will contain the text
   * @param element - The SVG element where text will be displayed
   */
  InitEmptyText(shape, element) {
    console.log("O.Opt: InitEmptyText input:", {
      shapeId: shape.BlockID || "unknown",
      elementType: element?.constructor?.name || "unknown"
    });

    // Get default text parameters
    const textParams = {};
    const defaultTextSettings = shape.GetTextDefault(textParams);
    const initialStyle = this.CalcDefaultInitialTextStyle(defaultTextSettings);
    const verticalAlignment = textParams.vjust;

    // Initialize with a space first to establish formatting
    element.textElem.SetText(' ');
    element.textElem.SetFormat(initialStyle);
    element.textElem.SetParagraphStyle(textParams);

    // Set vertical alignment for BaseShape objects
    if (shape instanceof Instance.Shape.BaseShape) {
      element.textElem.SetVerticalAlignment(verticalAlignment);
    }

    // Clear the text to make it truly empty
    element.textElem.SetText('');

    console.log("O.Opt: InitEmptyText complete for shape:", shape.BlockID || "unknown");
  }
