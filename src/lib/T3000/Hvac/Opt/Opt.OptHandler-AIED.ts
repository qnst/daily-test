...
class OptHandler {
  ...
  LM_SetupMove(event) {
    ...
    if (drawingObject.objecttype === ConstantData.ObjectTypes.SD_OBJT_GANTT_BAR) {
      this.moveBounds = drawingObject.GetMoveRect(false, false);
  ...
  MaintainLink(targetId, drawingObject, changeEvent, triggerType, maintainMode) {
    ...
    let connectionPoint = {};
    let targetPoints = [];
    const links = this.GetObjectPtr(this.linksBlockId, true);
  ...
  DragDropObjectDone(event, additionalData) {
    ...
    try {
      ...
      if (this.moveList && this.moveList.length) {
        ...
        for (index = 0; index < objectCount; index++) {
          Collab.AddNewBlockToSecondary(this.moveList[index]);

          currentObject = this.GetObjectPtr(this.moveList[index], true);
          if (currentObject) {
      ...
      if (this.moveList && this.moveList.length) {
        objectsToSelect = this.moveList.slice(0);
        this.actionStoredObjectId = -1;
      } else {
        this.AddToDirtyList(this.actionStoredObjectId);
      }
  ...
  Redo(shouldCancelModalOperation) {
    ...
    // Update page settings if changed
    if (T3Gv.docHandler.PagesNotEqual(updatedSessionData.Page, T3Gv.optManager.contentHeader.Page)) {
  ...
  CopyObjectsCommon(returnBuffer: boolean): { zList: any[]; buffer: string } | void {
    ...
    if (selectedCount !== 0) {
      ...
      this.contentHeader.ClipboardBuffer = SDF.WriteSelect(sortedObjects, false, true, false);
      this.contentHeader.ClipboardType = ConstantData.ClipboardType.LM;
