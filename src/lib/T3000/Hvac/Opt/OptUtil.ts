...
import Collaboration from "../Data/Constant/Collaboration";
...
class OptUtil {
      ...
      if (T3Gv.state.CurrentStateID >= 0) {
        ...
        if (!shouldKeepStateOpen && isStateOpen) {
          if (this.GetDocDirtyState()) {
            ...
            ShapeDataUtil.SaveChangedBlocks(T3Gv.state.CurrentStateID, 1);
          } else {
  ...
  StartRubberBandSelect(event: any) {
    ...
    try {
      ...
      console.log('O.Opt StartRubberBandSelect - Output rubber band set successfully:', T3Gv.opt.rubberBand);
    } catch (error) {
  ...
  RubberBandSelectMoveCommon(mouseX: number, mouseY: number) {
    ...
    console.log('O.Opt RubberBandSelectMoveCommon - Output:', { rubberBandFrame: T3Gv.opt.rubberBandFrame });
  }
  ...
  AddNewObject(drawingObject, shouldStyleCopy, renderSelection, textContent) {
    ...
    const newBlock = T3Gv.stdObj.CreateBlock(ConstantData.StoredObjectType.BASE_LM_DRAWING_OBJECT, drawingObject);
    if (newBlock == null) {
  ...
  LM_MoveClick(event) {
    ...
    try {
      ...
      if (this.currentModalOperation === ConstantData2.ModalOperations.NONE) {
        T3Gv.opt.SetEditMode(ConstantData.EditState.DRAGSHAPE);
      }
 ...
 */
  ...
  PolyLJoin(e, t, a, r, i) {
    var n, o, s, l, S, c, u, p, d, D, g, h, m, C, y, f = null, L = null, I = null, T = null, b = [], M = [], P = !1, R = [], A = !1, _ = ConstantData.ActionTriggerType.LINEEND, E = {
      x: 0,
      y: 0
    }, w = {}, F = ConstantData.Defines.SED_KnobSize, v = -1, G = T3Gv.opt.ActiveVisibleZList(), N = !1;
    ...
    if (objectIds != null) {
      ...
      for (objectIndex = objectCount - 1; objectIndex >= 0; objectIndex--) {
        ...
        if (objectBlock) {
          ...
          if (
            objectData.extraflags & ConstantData.ExtraFlags.SEDE_NoDelete && !forceDelete
          ) {
            continue;
          }
        ...
        if (currentObj.DrawingObjectBaseClass === ConstantData.DrawingObjectBaseClass.CONNECTOR) {
              ...
              if (this.IsConnectorEndShape(currentObj, tempObj, connectorEndInfo)) {
                ...
                if (objectIds.indexOf(parentConnector) < 0) {
                  if (connectorHookCount - connectorUsageCount > 1 || connectorEndInfo.pasted || isForced) {
                    ...
                    if ((tempObj.extraflags & ConstantData.ExtraFlags.SEDE_NoDelete)) {
          ...
          if (!flagSkip) {
            ...
            if (currentObj.DrawingObjectBaseClass === ConstantData.DrawingObjectBaseClass.LINE) {
              let childShapeList = T3Gv.opt.FindAllChildObjects(currentObj.BlockID, ConstantData.DrawingObjectBaseClass.SHAPE, ConstantData.ObjectTypes.SD_OBJT_MULTIPLICITY);
            ...
            if (!isForced) {
              let childLineList = T3Gv.opt.FindAllChildObjects(currentObj.BlockID, ConstantData.DrawingObjectBaseClass.LINE, null);
  ...
  IsGroupNonDelete(): boolean {
    ...
    for (let index = 0; index < selectedObjects.length; index++) {
      ...
      if (
        ...
        T3Gv.opt.GetObjectPtr(currentObject.hooks[0].objid, false).objecttype === ConstantData.ObjectTypes.SD_OBJT_TIMELINE
      ) {
        console.log("O.Opt IsGroupNonDelete - Output: true");
        return true;
      }
