...
FloorPlan.prototype.StopAddingWalls = function (e) {
  ...
  if (this.IsAddingWalls()) {
    this.ToggleAddingWalls(),
      (
        r = T3Gv.optManager.GetObjectPtr(T3Gv.optManager.theSelectedListBlockID, !1)
      ) &&
        r.length > 0 ? (
        T3Gv.optManager.ResetObjectDraw(),
        T3Constant.DocContext.SelectionToolSticky = !1,
    ...
    (
      ...
      r.length > 0 &&
      (a = Utils1.DeepCopy(r), T3Gv.optManager.SelectObjects(a))
  } else switch (T3Gv.optManager.currentModalOperation) {
    case t.ADDCORNER:
    case t.SPLITWALL:
