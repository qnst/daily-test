...
class ShapeDataUtil {
  ...
  static WriteTextParams(dataStream, drawingObject, textId, options) {
    ...
    ShapeDataUtil.Write_LENGTH(dataStream, lengthPosition);
  }
  ...
  static Result = function () {
    this.error = 0,
      ...
      this.ReadingGroup = !1,
      this.WindowSettings = new ShapeDataUtil.WindowSettings,
      this.DefTStyle = {},
      this.DefRun = {},
      this.DefFSize = 10,
      this.DefLine = {},
  ...
  static ReadBuffer(buffer, result, offset, ignoreErrors, callback) {
    ...
    if (result.ValidateHashesAsync && callback) {
      ...
      ShapeDataUtil.ReadBuffer_Complete(fileHeader, result, ignoreErrors);
      return result.error;
    }
  }
    ...
    try {
      ...
      for (codeIndex = 1; parsedData.codes[codeIndex].code != opCodes.cEndFile; codeIndex++) {
        switch (parsedData.codes[codeIndex].code) {
          ...
          case opCodes.SDF_C_DRAW12:
            codeIndex = ShapeDataUtil.ReadDraw(parsedData, codeIndex, result, opCodes, opCodes.SDF_C_DRAW12_END);
            if (codeIndex < 0) break;
            break;

          case opCodes.SDF_C_DRAW8:
            codeIndex = ShapeDataUtil.ReadDraw(parsedData, codeIndex, result, opCodes, opCodes.SDF_C_DRAW8_END);
            if (codeIndex < 0) break;
            break;

          case opCodes.SDF_C_DRAW:
            codeIndex = ShapeDataUtil.ReadDraw(parsedData, codeIndex, result, opCodes, opCodes.SDF_C_DRAW_END);
            if (codeIndex < 0) break;
            break;
      ...
      if (result.isSymbol === false) {
        if (result.VisioFileVersion) {
          T3Gv.optManager.FileVersion = ShapeDataUtil.SDF_FVERSION2022;
        }
    ...
    while (codeData.codes[codeIndex].code != endCodeMarker) {
      switch (codeData.codes[codeIndex].code) {
        ...
        case opCodes.SDF_C_GRAPH:
          ...
          codeIndex = ShapeDataUtil.ReadGraph(graphBlock, codeData, codeIndex, resultObject, opCodes, opCodes.SDF_C_GRAPH_END);
          break;
    ...
    for (t++, n && u && (n.EMFHash = u); e.codes[t].code != i;) {
      switch (e.codes[t].code) {
        ...
        case r.SDF_C_TEXT:
          ...
          t = ShapeDataUtil.ReadText(l, null, null, e, t, a, r, !1, r.SDF_C_TEXT_END),
            a.textonline >= 0 &&
            (
        ...
        case r.SDF_C_COMMENT:
          a.ReadBlocks ||
            a.ReadGroupBlock ? t++ : t = ShapeDataUtil.ReadText(n, null, null, e, t, a, r, !0, r.SDF_C_COMMENT_END);
          break;
        ...
        case r.SDF_C_GRAPH:
          h = new Instance.Shape.Graph,
            n.SetGraph(h),
            t = ShapeDataUtil.ReadGraph(h, e, t, a, r, r.SDF_C_GRAPH_END);
          break;
  ...
  static WriteHeader(dataStream, resultObject, skipCodes) {
    ...
    ShapeDataUtil.write_SDF_C_HEADER(dataStream, resultObject);
    ShapeDataUtil.write_SDF_C_PAGE(dataStream, resultObject);
    ...
    if (skipCodes == null) {
      ...
      ShapeDataUtil.WriteString(dataStream, exportPath, opCodes.SDF_C_EXPORTPATH, resultObject);
    }
  ...
  static WriteString(dataStream, stringValue, opCode, resultObject) {
    if (stringValue != null && stringValue.length) {
      const codeOffset = ShapeDataUtil.Write_CODE(dataStream, opCode);
  ...
  static WriteLongValue(dataStream, operationCode, longValue, resultObject) {
    const codeOffset = ShapeDataUtil.Write_CODE(dataStream, operationCode);
    const longValueData = {
      value: longValue
    };
  ...
  static WriteDraw(dataStream, resultObject) {
    ...
    ShapeDataUtil.write_SDF_C_DRAW12(dataStream, resultObject);
    ...
    ShapeDataUtil.WriteTextureList(dataStream, T3Gv.optManager.TextureList, resultObject);
  ...
  static WritePaint(dataStream, paintData, defaultColor, resultObject) {
    ...
    switch (paintData.FillType) {
      ...
      case ConstantData.FillTypes.SDFILL_TEXTURE:
        ...
        ShapeDataUtil.Write_LENGTH(dataStream, textureOffset);
        break;
    }
  ...
  static WriteObject(e, t, a, r) {
    ...
    if (
      ...
      a.HyperlinkText &&
      ShapeDataUtil.WriteString(e, a.HyperlinkText, L.SDF_C_DRAWJUMP, r),
      r.WriteBlocks ||
      r.WriteGroupBlock ||
  ...
  static BlockHeader = function (e, t, a, r, i, n) {
    this.state = e,
      this.delta = t,
      this.action = ShapeDataUtil.BlockActions.Normal,
      this.blocktype = a,
      this.blockid = r,
      this.index = i,
      this.nblocks = n
  ...
  static SaveChangedBlocks(stateId, deltaState, targetStateId, customStoredObjects) {
    try {
      ...
      const resultObject = new ShapeDataUtil.WResult();
      ...
      if (T3Gv.gTestException) {
        const error = new Error("in complete");
        error.name = '1';
        throw error;
      }
