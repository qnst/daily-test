...
import PolygonShapeGenerator from "./PolygonUtil"
...
class ShapeDataUtil {
  ...
  static WriteTextParams(dataStream, drawingObject, textId, options) {
    ...
    const winRect = ShapeDataUtil.ToSDWinRect(objectToWrite.trect, options.coordScaleFactor, options.GroupOffset);

    // Convert text alignment to Windows format
    const winJustification = ShapeDataUtil.TextAlignToWin(drawingObject.TextAlign);

    // Calculate text wrap width if specified
    let textWrapWidth = 0;
    if (drawingObject.TextWrapWidth > 0) {
      textWrapWidth = ShapeDataUtil.ToSDWinCoords(drawingObject.TextWrapWidth, options.coordScaleFactor);
    }

    // Write the text code and reserve space for length
    const lengthPosition = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cDrawText);
    ...
    if (options.WriteWin32) {
      ...
      const textParams = {
        ...
        tindent: {
          left: ShapeDataUtil.ToSDWinCoords(drawingObject.tindent.left, options.coordScaleFactor),
          top: ShapeDataUtil.ToSDWinCoords(drawingObject.tindent.top, options.coordScaleFactor),
          right: ShapeDataUtil.ToSDWinCoords(drawingObject.tindent.right, options.coordScaleFactor),
          bottom: ShapeDataUtil.ToSDWinCoords(drawingObject.tindent.bottom, options.coordScaleFactor)
        },
        tmargin: {
          left: ShapeDataUtil.ToSDWinCoords(drawingObject.TMargins.left, options.coordScaleFactor),
          top: ShapeDataUtil.ToSDWinCoords(drawingObject.TMargins.top, options.coordScaleFactor),
          right: ShapeDataUtil.ToSDWinCoords(drawingObject.TMargins.right, options.coordScaleFactor),
          bottom: ShapeDataUtil.ToSDWinCoords(drawingObject.TMargins.bottom, options.coordScaleFactor)
        },
        textid: textId,
        ...
        tangle: ShapeDataUtil.ToWinAngle(drawingObject.RotationAngle),
        gtangle: 0,
        ltrect: {
          left: winRect.left,
          top: winRect.top,
        ...
        linetexty: ShapeDataUtil.ToSDWinCoords(drawingObject.LineTextY, options.coordScaleFactor),
    ...
    ShapeDataUtil.WriteLength(dataStream, lengthPosition);
  }
  ...
  static ReadSymbolFromBuffer(buffer, positionX, positionY, offset, ignoreErrors, renderObjects,
    ...
    let errorCode = ShapeDataUtil.ReadBuffer(buffer, result, offset, false, ShapeDataUtil.ReadSymbolFromBuffer_Complete);
    if (errorCode && errorCode != ShapeDataUtil.Errors.WaitingForCallBack) {
      return ignoreErrors ? result.error : errorCode;
    }

    if (result.WarnMeta) {
      if (ignoreErrors) return ShapeDataUtil.Errors.WarnMeta;
      alert('Metafile not read');
    }

    if (outputDimensions && errorCode !== ShapeDataUtil.Errors.WaitingForCallBack) {
    ...
    if (errorCode !== ShapeDataUtil.Errors.WaitingForCallBack) {
      ...
      for (index = 0; index < objectCount; index++) {
        ...
        if (object.objecttype === ConstantData.ObjectTypes.SD_OBJT_BPMN_POOL) {
          ShapeDataUtil.ConvertBPMNPool(object);
        }
   ...
   * @returns Error code if an error occurred, or ShapeDataUtil.Errors.WaitingForCallBack for async operations
   */
  static ReadBuffer(buffer, result, offset, ignoreErrors, callback) {
    ...
    if (result.isSymbol) {
      minimumFileVersion = ShapeDataUtil.SDF_MINSVERSION;
    }
    ...
    if (!fileHeader || !fileHeader.start || fileHeader.start != ShapeDataUtil.Signature) {
    ...
    if (fileHeader.codes[0].data.MinVer > ShapeDataUtil.SDF_FVERSION) {
      result.error = ShapeDataUtil.Errors.Version;
      return result.error;
    }

    // Check if file version is too old
    if (fileHeader.codes[0].data.FVersion < minimumFileVersion) {
      result.error = ShapeDataUtil.Errors.MinVersion;
      return result.error;
    }

    // Set conversion flags based on file version
    if (fileHeader.codes[0].data.FVersion < ShapeDataUtil.SDF_FVERSION) {
      result.ConvertOnSave = true;
      if (fileHeader.codes[0].data.FVersion <= ShapeDataUtil.FVERSIONVSM &&
    ...
    if (result.FVersion < ShapeDataUtil.SDF_FVERSION2022) {
    ...
    if (result.ValidateHashesAsync && callback) {
      ...
      return ShapeDataUtil.ValidateHashCodes(null, fileHeader, opCodes, result, ignoreErrors, callback) ||
        ShapeDataUtil.Errors.WaitingForCallBack;
    } else {
      // Process synchronously
      ShapeDataUtil.ReadBufferComplete(fileHeader, result, ignoreErrors);
      return result.error;
    }
  }
   ...
   * @returns Error code if an error occurred, 0 otherwise
   */
  static ReadBufferComplete(parsedData, result, ignoreErrors) {
    try {
      const opCodes = ShapeConstant.OpNameCode;
      ...
      for (codeIndex = 1; parsedData.codes[codeIndex].code != opCodes.cEndFile; codeIndex++) {
        switch (parsedData.codes[codeIndex].code) {
          ...
          case opCodes.cSdData64:
            if (!dataBlockLoaded && result.PVersion >= ShapeDataUtil.SDF_PVERSION861) {
          ...
          case opCodes.cDraw12:
            codeIndex = ShapeDataUtil.ReadDraw(parsedData, codeIndex, result, opCodes, opCodes.cDraw12End);
            if (codeIndex < 0) break;
            break;

          case opCodes.cDraw8:
            codeIndex = ShapeDataUtil.ReadDraw(parsedData, codeIndex, result, opCodes, opCodes.cDraw8End);
            if (codeIndex < 0) break;
            break;

          case opCodes.cDraw:
            codeIndex = ShapeDataUtil.ReadDraw(parsedData, codeIndex, result, opCodes, opCodes.cDrawEnd);
            if (codeIndex < 0) break;
            break;

          // Process any other blocks with begin/end structure
          default:
            if (parsedData.codes[codeIndex].code & ShapeDataUtil.SDF_BEGIN) {
              codeIndex = ShapeDataUtil.ReadFrame(
                parsedData,
                codeIndex,
                (parsedData.codes[codeIndex].code & ShapeDataUtil.SDF_MASK) | ShapeDataUtil.SDF_END
              );
            }
        }
      ...
      if (result.error === 0) {
        ShapeDataUtil.ReMapLinks(result.IDMap, result.links, result, ignoreErrors);
      }
      ...
      for (objectIndex = 0; objectIndex < objectCount; objectIndex++) {
        ...
        if (result.PVersion < ShapeDataUtil.SDF_PVERSION861 &&
          object.objecttype === ConstantData.ObjectTypes.SD_OBJT_GANTT_CHART) {
          result.error = ShapeDataUtil.Errors.MinVersionProjectChart;
          return -1;
        }
        ...
        switch (baseClass) {
          case ConstantData.DrawingObjectBaseClass.LINE:
            ...
            if (object.DataID >= 0 && result.PVersion < ShapeDataUtil.SDF_PVERSION859 && result.ReadBlocks) {
        ...
        // Handle text formatting for Visio objects
          ...
          if (svgElement) {
            ...
            if (result.IsVisio &&
              ...
              const textAlignment = ShapeDataUtil.TextAlignToJust(object.TextAlign);
      ...
      if (result.isSymbol === false) {
        if (result.VisioFileVersion) {
          T3Gv.opt.FileVersion = ShapeDataUtil.SDF_FVERSION2022;
        }
            ...
            if (idMap[currentObject.hooks[currentHook].objid] &&
              ...
              if (resultObject.IsVisio &&
                ...
                if (targetObject &&
                  ...
                  ShapeDataUtil.ConnectedToEdge(currentObject.hooks[currentHook].connect)) {
          ...
          for (currentHook = 0; currentHook < hookCount; currentHook++) {
            ...
            if (currentObject.arraylist.hook[currentHook].textid >= 0) {
              if (!resultObject.ReadBlocks && !resultObject.ReadGroupBlock) {
                currentObject.arraylist.hook[currentHook].textid = ShapeDataUtil.GetLineText(
                  resultObject,
                  idIndex,
  ...
  static ReadOutSide(outsideData, isVisio) {
    ...
    effectData.Color = ShapeDataUtil.WinColorToHTML(outsideData.color);
    ...
    while (codeData.codes[codeIndex].code != opCodes.cEndFill) {
      switch (codeData.codes[codeIndex].code) {
        case opCodes.cBeginPaint:
          ...
          codeIndex = ShapeDataUtil.ReadPaint(fillObject.Paint, codeData, codeIndex, resultObject, opCodes);
          break;
        ...
        case opCodes.cEffect:
          ...
          fillObject.EffectColor = ShapeDataUtil.WinColorToHTML(codeData.codes[codeIndex].data.effectcolor);
        ...
        default:
          ...
          if (codeData.codes[codeIndex].code & ShapeDataUtil.SDF_BEGIN) {
            codeIndex = ShapeDataUtil.ReadFrame(
              codeData,
              codeIndex,
              codeData.codes[codeIndex].code & ShapeDataUtil.SDF_MASK | ShapeDataUtil.SDF_END
            );
          }
      }
      codeIndex++;
  ...
  static ReadSDLine(lineStyleObject, codeData, codeIndex, resultObject, opCodes) {
    ...
    lineStyleObject.Thickness = ShapeDataUtil.ToSDJSCoords(
    ...
    while (codeData.codes[codeIndex].code != opCodes.cEndLine) {
      switch (codeData.codes[codeIndex].code) {
        case opCodes.cBeginPaint:
          ...
          codeIndex = ShapeDataUtil.ReadPaint(
          ...
          switch (linePattern) {
            ...
            case linePatterns.SEP_FilledLine:
              ...
              if (lineStyleObject.Paint.Color === ConstantData.Colors.Color_White &&
                originalThickness > 0) {
                colorValue = ShapeDataUtil.WinColorToHTML(codeData.codes[codeIndex].data.color);
                if (colorValue != ConstantData.Colors.Color_White) {
                  lineStyleObject.Paint.Color = colorValue;
                  lineStyleObject.Paint.Opacity = ShapeDataUtil.WinColorToAlpha(
  ...
  static ReadPaint(paintObject, codeData, codeIndex, resultObject, opCodes) {
    ...
    paintObject.Color = ShapeDataUtil.WinColorToHTML(codeData.codes[codeIndex].data.color);
    paintObject.Opacity = ShapeDataUtil.WinColorToAlpha(codeData.codes[codeIndex].data.color);
    ...
    while (codeData.codes[codeIndex].code != opCodes.cEndPaint) {
      switch (codeData.codes[codeIndex].code) {
        case opCodes.cGradient:
          ...
          paintObject.EndColor = ShapeDataUtil.WinColorToHTML(codeData.codes[codeIndex].data.ecolor);
          paintObject.GradientFlags = codeData.codes[codeIndex].data.gradientflags;
          paintObject.EndOpacity = ShapeDataUtil.WinColorToAlpha(codeData.codes[codeIndex].data.ecolor);
  ...
  static ReadSDTxf(textFormatObject, codeData, codeIndex, resultObject, opCodes) {
    ...
    if (resultObject.fontlist.length) {
      const fontRecord = ShapeDataUtil.FontIDtoFontRec(codeData.codes[codeIndex].data.fontid, resultObject);
    ...
    while (codeData.codes[codeIndex].code != opCodes.cEndTextf) {
      switch (codeData.codes[codeIndex].code) {
        ...
        case opCodes.cOutSide:
          ...
          textFormatObject.Effect = ShapeDataUtil.ReadOutSide(
    ...
    while (codeData.codes[codeIndex].code != opCodes.cEndStyle) {
      switch (codeData.codes[codeIndex].code) {
        case opCodes.cBeginFill:
          ...
          codeIndex = ShapeDataUtil.ReadSDFill(styleObject.Fill, codeData, codeIndex, resultObject, opCodes);
          break;

        case opCodes.cBeginLine:
          // Process both border and line properties
          ShapeDataUtil.ReadSDLine(styleObject.Border, codeData, codeIndex, resultObject, opCodes);
          codeIndex = ShapeDataUtil.ReadSDLine(styleObject.Line, codeData, codeIndex, resultObject, opCodes);
          break;

        case opCodes.cBeginTextf:
          // Process text formatting properties
          codeIndex = ShapeDataUtil.ReadSDTxf(styleObject.Text, codeData, codeIndex, resultObject, opCodes);
          break;

        case opCodes.cOutSide:
          // Process outside effects (shadows, outlines, etc.)
          styleObject.OutsideEffect = ShapeDataUtil.ReadOutSide(codeData.codes[codeIndex].data, resultObject.IsVisio);
          break;
  ...
  static ReadStyleList(styleArray, codeData, codeIndex, resultObject, opCodes) {
    ...
    while (codeData.codes[codeIndex].code != opCodes.cEndStyleList) {
      if (codeData.codes[codeIndex].code === opCodes.cBeginStyle) {
        ...
        codeIndex = ShapeDataUtil.ReadStyle(styleObject, codeData, codeIndex, resultObject, opCodes);
        styleArray.push(styleObject);
      } else if (codeData.codes[codeIndex].code & ShapeDataUtil.SDF_BEGIN) {
        // Handle nested structures by recursively reading frames
        codeIndex = ShapeDataUtil.ReadFrame(
          codeData,
          codeIndex,
  ...
  static SetCurvatureDefaults(sessionObject, resultObject) {
    ...
    if (!resultObject.isSymbol &&
      ...
      resultObject.PVersion < ShapeDataUtil.SDF_PVERSION864) {
  ...
  static ReadDraw(codeData, codeIndex, resultObject, opCodes, endCodeMarker) {
    ...
    if (endCodeMarker != opCodes.cDrawEnd) {
      ...
      ShapeDataUtil.ReadDrawSession(sessionData, layerManager, codeData.codes[codeIndex].data, resultObject);

      // Apply special handling for Visio imports
      if (resultObject.IsVisio) {
        ShapeDataUtil.SetDefaults(sessionData, resultObject);
      }
    } else {
      // Legacy format (version 6) - read and convert session data
      ShapeDataUtil.SetDefaults(sessionData, resultObject);
      ShapeDataUtil.ReadDrawSession6(sessionData, codeData.codes[codeIndex].data, resultObject);
      ShapeDataUtil.SetDefaults(sessionData, resultObject);
    }

    // Apply additional defaults and fixes
    ShapeDataUtil.FixDefaults(sessionData, resultObject);
    ShapeDataUtil.SetCurvatureDefaults(sessionData, resultObject);
    ...
    while (codeData.codes[codeIndex].code != endCodeMarker) {
      switch (codeData.codes[codeIndex].code) {
        case opCodes.cDraw7:
          ...
          ShapeDataUtil.ReadDraw7(sessionData, layerManager, codeData.codes[codeIndex].data, resultObject);
          break;

        case opCodes.cDrawExtra:
          // Process extended drawing properties
          ShapeDataUtil.ReadDrawExtra(sessionData, codeData.codes[codeIndex].data, resultObject);
          break;
        ...
        case opCodes.cBeginStyle:
          ...
          if (styleCount === 0) {
            ...
            codeIndex = ShapeDataUtil.ReadStyle(sessionData.def.style, codeData, codeIndex, resultObject, opCodes);
            ShapeDataUtil.SetDefaults(sessionData, resultObject);
          } else {
            // Additional styles
            let newStyle = new QuickStyle();
            codeIndex = ShapeDataUtil.ReadStyle(newStyle, codeData, codeIndex, resultObject, opCodes);
          }
          styleCount++;
          break;
        ...
        case opCodes.cBeginFill:
          ...
          codeIndex = ShapeDataUtil.ReadSDFill(sessionData.background, codeData, codeIndex, resultObject, opCodes);
        ...
        case opCodes.cBeginLine:
          ...
          codeIndex = ShapeDataUtil.ReadSDLine(sessionData.def.style.Line, codeData, codeIndex, resultObject, opCodes);
          ShapeDataUtil.SetDefaults(sessionData, resultObject);
          break;

        case opCodes.cBeginStyleList:
          // Process list of styles
          codeIndex = ShapeDataUtil.ReadStyleList(resultObject.lpStyles, codeData, codeIndex, resultObject, opCodes);
          break;

        case opCodes.cDrawLink:
        case opCodes.cDrawLink6:
          // Process object links
          ShapeDataUtil.ReadLinkList(resultObject.links, codeData.codes[codeIndex].data);
          break;
        ...
        case opCodes.cTableBlock:
          // Process table block
          tableBlock = ShapeDataUtil.ReadTableBlock(codeData, codeIndex, resultObject);
          break;

        case opCodes.cGraphBlock:
          // Process graph block
          graphBlock = ShapeDataUtil.ReadGraphBlock(codeData, codeIndex, resultObject);
          break;
        ...
        case opCodes.cGraph:
          ...
          codeIndex = ShapeDataUtil.ReadGraph(graphBlock, codeData, codeIndex, resultObject, opCodes, opCodes.cGraphEnd);
          break;

        case opCodes.cExpandedViewBlock:
          // Process expanded view block
          expandedViewBlock = ShapeDataUtil.ReadExpandedViewBlock(codeData, codeIndex, resultObject);
          break;
        ...
        case opCodes.cGanttInfoBlock:
          ...
          ganttInfoBlock = ShapeDataUtil.ReadGanttInfoBlock(codeData, codeIndex, resultObject);
          break;

        case opCodes.cCloudCommentBlock:
          // Process cloud comment block
          ShapeDataUtil.ReadCommentBlock(codeData, codeIndex, resultObject);
          break;
        ...
        case opCodes.cGanttInfo:
          ...
          ShapeDataUtil.ReadGanttInfo(ganttInfoBlock, codeData.codes[codeIndex].data, resultObject);
          break;

        case opCodes.cLongText8:
          // Process text block
          codeIndex = ShapeDataUtil.ReadTextBlock(codeData, codeIndex, resultObject, opCodes, false);
          break;

        case opCodes.cComment:
          // Process comment text
          codeIndex = ShapeDataUtil.ReadTextBlock(codeData, codeIndex, resultObject, opCodes, true);
          break;

        case opCodes.cImageBlock:
          // Process image block
          ShapeDataUtil.ReadImageBlock(codeData, codeIndex, resultObject, opCodes, true);
          break;

        case opCodes.cNativeBlock:
          // Process native format block
          codeIndex = ShapeDataUtil.ReadNativeBlock(codeData, codeIndex, resultObject, opCodes, true);
          break;

        case opCodes.cNativeWinBlock:
          // Process native Windows format block
          codeIndex = ShapeDataUtil.ReadNativeBlock(codeData, codeIndex, resultObject, opCodes, false);
          break;

        case opCodes.oRuler:
          // Process ruler configuration
          resultObject.rulerConfig = new RulerConfig();
          ShapeDataUtil.ReadRulers(codeData.codes[codeIndex].data, resultObject);
          break;

        case opCodes.cLineDrawList:
          // Process line drawing list
          ShapeDataUtil.ReadLineDrawList(codeData.codes[codeIndex].data, resultObject);
          break;

        case opCodes.cRecentSymbolsBegin:
          // Process recent symbols list
          codeIndex = ShapeDataUtil.ReadRecentSymbols(codeData, codeIndex, resultObject, opCodes);
          break;

        case opCodes.cBeginLayer:
          // Process layer definitions
          codeIndex = ShapeDataUtil.ReadLayers(codeData, codeIndex, resultObject, opCodes);
          break;
        ...
        case opCodes.cDrawImage8:
          ...
          if (resultObject.PVersion >= ShapeDataUtil.SDF_PVERSION838 &&
        ...
        case opCodes.oTextureList:
          ...
          codeIndex = ShapeDataUtil.ReadTextureList(codeData, codeIndex, resultObject, opCodes, false);
          break;
  ...
  static ReadDrawSession(sessionObject, layerManager, sourceData, resultObject) {
    ...
    sessionObject.dim.x = ShapeDataUtil.ToSDJSCoords(dimensions.x, resultObject.coordScaleFactor);
    sessionObject.dim.y = ShapeDataUtil.ToSDJSCoords(dimensions.y, resultObject.coordScaleFactor);
    ...
    sessionObject.dupdisp.x = ShapeDataUtil.ToSDJSCoords(displacement.x, resultObject.coordScaleFactor);
    ...
    sessionObject.def.vjust = ShapeDataUtil.W32JustToJS(sourceData.vjust, true);
    ...
    sessionObject.hopdim.x = ShapeDataUtil.ToSDJSCoords(sourceData.hopdim.x, resultObject.coordScaleFactor);
    sessionObject.hopdim.y = ShapeDataUtil.ToSDJSCoords(sourceData.hopdim.y, resultObject.coordScaleFactor);
    ...
    sessionObject.def.tmargins.left = ShapeDataUtil.ToSDJSCoords(sourceData.tmargins.left, resultObject.coordScaleFactor);
    sessionObject.def.tmargins.top = ShapeDataUtil.ToSDJSCoords(sourceData.tmargins.top, resultObject.coordScaleFactor);
    sessionObject.def.tmargins.right = ShapeDataUtil.ToSDJSCoords(sourceData.tmargins.right, resultObject.coordScaleFactor);
    sessionObject.def.tmargins.bottom = ShapeDataUtil.ToSDJSCoords(sourceData.tmargins.bottom, resultObject.coordScaleFactor);
    ...
    if (sourceData.h_arraywidth) {
      sessionObject.def.h_arraywidth = ShapeDataUtil.ToSDJSCoords(sourceData.h_arraywidth, resultObject.coordScaleFactor);
      sessionObject.def.v_arraywidth = ShapeDataUtil.ToSDJSCoords(sourceData.v_arraywidth, resultObject.coordScaleFactor);
    }

    if (sourceData.arrayht) {
      sessionObject.def.arraywd = ShapeDataUtil.ToSDJSCoords(sourceData.arraywd, resultObject.coordScaleFactor);
      sessionObject.def.arrayht = ShapeDataUtil.ToSDJSCoords(sourceData.arrayht, resultObject.coordScaleFactor);
    }
  ...
  static SetDefaults(session, defaults) {
    ...
    // Set text defaults
    ShapeDataUtil.DefaultText(session, defaults);
  }
  ...
  static ReadRulers(rulerData, resultObject) {
    ...
    resultObject.rulerConfig.major = ShapeDataUtil.ToSDJSCoords(rulerData.Major, resultObject.coordScaleFactor);

    // Handle version-specific scaling
    if (resultObject.PVersion < ShapeDataUtil.SDF_POVERSION801) {
  ...
  static ReadObject(e, t, a, r, i) {
    ...
    if (
      ...
      (
        ...
        ShapeDataUtil.ObjectIsConnectorTextLabel(e, t, r, i) ||
        (
          e.codes[t].data.objecttype === ConstantData.ObjectTypes.SD_OBJT_NG_EVENT_LABEL ||
            ...
            e.codes[t].data.objecttype === ConstantData.ObjectTypes.SD_OBJT_MANUAL_EVENT_LABEL ? m = !0 : (m = ShapeDataUtil.ObjectIsExternalTextLabel(e, t, r, i, a)) &&
      ...
      !(
        n = ShapeDataUtil.ReadObjectHeader(
          w,
          F,
          e.codes[t].data,
          a,
    ...
    if (a.LineTextObject) {
      ...
      if (!a.ReadBlocks || a.BlockzList.indexOf(n.UniqueID) >= 0) {
        var N = T3Gv.stdObj.CreateBlock(ConstantData.StoredObjectType.BASE_LM_DRAWING_OBJECT, n);
        n = N.Data,
          a.zList.push(N.ID),
      ...
      n.DrawingObjectBaseClass === ConstantData.DrawingObjectBaseClass.LINE &&
        (
          b = {},
          (c = ShapeDataUtil.GetLineText(a, a.objectcount, null, b)) >= 0 &&
          (
            n.DataID = c,
            a.IsVisio &&
    ...
    for (t++, n && u && (n.EMFHash = u); e.codes[t].code != i;) {
      switch (e.codes[t].code) {
        ...
        case r.cSdData64:
          break;
        case r.cDrawSegl:
          if (ShapeDataUtil.ReadSegl(n, e.codes[t].data, a)) return - 1;
          break;
        case r.cDrawPoly:
          if ((t = ShapeDataUtil.ReadPolyLine(n, e, t, a, r)) < 0) return t;
          break;
        case r.cFreeHandLine:
          if (ShapeDataUtil.ReadFreehand(n, e.codes[t].data, a)) return - 1;
          break;
        case r.cDrawArray:
          if ((t = ShapeDataUtil.ReadArrayList(n, e, t, a, r)) < 0) return t;
          break;
        case r.cDrawContainer:
          if ((t = ShapeDataUtil.ReadContainerList(n, e, t, a, r)) < 0) return t;
          break;
        case r.cDrawHook:
          ShapeDataUtil.ReadHook(n, e.codes[t].data, a);
          break;
        case r.cBeginStyle:
          t = ShapeDataUtil.ReadStyle(n.StyleRecord, e, t, a, r);
          break;
        case r.cBeginLine:
          t = ShapeDataUtil.ReadSDLine(n.StyleRecord.Line, e, t, a, r);
          break;
        case r.cBeginFill:
          t = ShapeDataUtil.ReadSDFill(n.StyleRecord.Fill, e, t, a, r);
          break;
        case r.cBeginTextf:
          t = ShapeDataUtil.ReadSDTxf(n.StyleRecord.Text, e, t, a, r);
          break;
        case r.cOutSide:
          n.StyleRecord.OutsideEffect = ShapeDataUtil.ReadOutSide(e.codes[t].data, a.IsVisio);
          break;
        case r.cDrawArrow:
          if (a.error = ShapeDataUtil.ReadArrow(n, e.codes[t].data), a.error) return - 1;
          break;
        case r.cConnectPoint:
          ShapeDataUtil.ReadConnectPoints(n, e.codes[t].data);
          break;
        case r.cDrawText:
          ShapeDataUtil.ReadTextParams(n, e.codes[t].data, a),
            n.DrawingObjectBaseClass !== ConstantData.DrawingObjectBaseClass.LINE &&
        ...
        case r.cText:
          ...
          t = ShapeDataUtil.ReadText(l, null, null, e, t, a, r, !1, r.cTextEnd),
            a.textonline >= 0 &&
            (
        ...
        case r.cNativeStorage:
          if (o) {
            if (ShapeDataUtil.ReadGroup(n, e.codes[t].data, a)) return - 1
          } else if (G) break;
          break;
        case r.cNativeId:
        ...
        case r.cOleStorageId:
          ShapeDataUtil.ReadImageID(n, null, e.codes[t].data, a, E);
          break;
        ...
        case r.cEmfHash:
          n.EMFHash = e.codes[t].data.name,
            n.ShapeType === ConstantData.ShapeType.SVGFRAGMENTSYMBOL &&
              null == n.SVGFragment ? ShapeDataUtil.GetSVGFragment(a, n, n.EMFHash) : (
            ...
            null == n.EMFHash &&
              ...
              (
                n.ShapeType === ConstantData.ShapeType.SVGFRAGMENTSYMBOL ? null == n.SVGFragment &&
                  v &&
                  ShapeDataUtil.GetSVGFragment(a, n, n.EMFHash) : _ ? (
                    n.SymbolURL = Constants.FilePath_HashSVG + n.EMFHash,
                    n.SymbolURL = n.SymbolURL + '.svg'
                  ) : (
        ...
        case r.cComment:
          a.ReadBlocks ||
            a.ReadGroupBlock ? t++ : t = ShapeDataUtil.ReadText(n, null, null, e, t, a, r, !0, r.cCommentEnd);
          break;
        case r.cNativeEmbedStorage:
        case r.cMarkUp:
          break;
        case r.cDrawObj5:
          ShapeDataUtil.ReadObj5(n, e.codes[t].data, a);
          break;
        case r.cDrawObj6:
          ShapeDataUtil.ReadDraw6(n, e.codes[t].data, a);
          break;
        case r.cDrawObj7:
          ShapeDataUtil.ReadObj7(n, e.codes[t].data, a);
          break;
        case r.cDrawBorder:
          ShapeDataUtil.ReadBorder(n, e.codes[t].data, a);
          break;
        case r.cDrawLine:
          ShapeDataUtil.Readv6Line(n, e.codes[t].data, a);
          break;
        case r.cDrawFill:
          ShapeDataUtil.Readv6Fill(n, e.codes[t].data);
          break;
        case r.cGraph:
          h = new Instance.Shape.Graph,
            n.SetGraph(h),
            t = ShapeDataUtil.ReadGraph(h, e, t, a, r, r.cGraphEnd);
          break;
        ...
        case r.cGanttInfo:
          theGanttInfo = new TODO.Table.GanttInfo,
            ...
            ShapeDataUtil.ReadGanttInfo(theGanttInfo, e.codes[t].data, a);
          break;
        case r.cD3Settings:
          n.ImportD3Settings &&
          ...
          break;
        default:
          e.codes[t].code & ShapeDataUtil.SDF_BEGIN &&
            (
              t = ShapeDataUtil.ReadFrame(e, t, e.codes[t].code & ShapeDataUtil.SDF_MASK | ShapeDataUtil.SDF_END)
            )
      }
      t++
    }
    return n.DrawingObjectBaseClass === ConstantData.DrawingObjectBaseClass.SHAPE &&
      ...
      (c = ShapeDataUtil.GetLineText(a, a.objectcount, null, null)) >= 0 &&
      (n.DataID = c),
      t
  }
  ...
  static ReadTextParams(targetObject, sourceData, resultObject) {
    ...
    targetObject.trect = ShapeDataUtil.ToSDJSRect(textRect, resultObject.coordScaleFactor);
    ...
    targetObject.tindent.left = ShapeDataUtil.ToSDJSCoords(sourceData.tindent.left, resultObject.coordScaleFactor);
    targetObject.tindent.top = ShapeDataUtil.ToSDJSCoords(sourceData.tindent.top, resultObject.coordScaleFactor);
    targetObject.tindent.right = ShapeDataUtil.ToSDJSCoords(sourceData.tindent.right, resultObject.coordScaleFactor);
    targetObject.tindent.bottom = ShapeDataUtil.ToSDJSCoords(sourceData.tindent.bottom, resultObject.coordScaleFactor);

    // Set text margins with proper scaling
    targetObject.TMargins.left = ShapeDataUtil.ToSDJSCoords(sourceData.tmargin.left, resultObject.coordScaleFactor);
    targetObject.TMargins.top = ShapeDataUtil.ToSDJSCoords(sourceData.tmargin.top, resultObject.coordScaleFactor);
    targetObject.TMargins.right = ShapeDataUtil.ToSDJSCoords(sourceData.tmargin.right, resultObject.coordScaleFactor);
    targetObject.TMargins.bottom = ShapeDataUtil.ToSDJSCoords(sourceData.tmargin.bottom, resultObject.coordScaleFactor);
    ...
    targetObject.TextAlign = ShapeDataUtil.W32JustToTextAlign(sourceData.just, sourceData.vjust);
    ...
    if (sourceData.textwrapwidth > 0) {
      targetObject.TextWrapWidth = ShapeDataUtil.ToSDJSCoords(sourceData.textwrapwidth, resultObject.coordScaleFactor);
    }
    ...
    if (sourceData.linetexty !== undefined) {
      targetObject.LineTextY = ShapeDataUtil.ToSDJSCoords(sourceData.linetexty, resultObject.coordScaleFactor);
    }
    ...
    if (targetObject.moreflags & ConstantData.ObjMoreFlags.SED_MF_VisioLineTextLabel && resultObject.IsVisio) {
      ...
      targetObject.RotationAngle = ShapeDataUtil.ToSDJSAngle(sourceData.tangle);
    }
    ...
    if (resultObject.ReadBlocks || resultObject.ReadGroupBlock) {
      ...
      if (sourceData.textid >= 0) {
        ...
        if (textObject) {
          const textAlignment = ShapeDataUtil.TextAlignToJust(targetObject.TextAlign);
  ...
  static CreateShapeObject(shapeConfig, shapeData, resultObject, extraFlags) {
    ...
    switch (shapeData.dataclass) {
      case PolygonConstant.ShapeTypes.RECTANGLE:
        ShapeDataUtil.SetCurvature(resultObject, shapeConfig, false);
      // fall through
      ...
      case PolygonConstant.ShapeTypes.PARALLELOGRAM:
        shapeType = PolygonConstant.ShapeTypes.POLYGON;
        shapeParam = ShapeDataUtil.ToSDJSCoords(shapeConfig.shapeparam, resultObject.coordScaleFactor);
      ...
      case PolygonConstant.ShapeTypes.STORAGE:
        shapeType = PolygonConstant.ShapeTypes.POLYGON;
        scaleFactor = ShapeDataUtil.ToSDJSCoords(shapeConfig.shapeparam, resultObject.coordScaleFactor);
  ...
  static ReadObjectHeader(sessionObject, layerManager, sourceData, resultObject, isGroup, isSymbol, skipAssociationCheck, applyStyles) {
    ...
    objectConfig.Frame = ShapeDataUtil.ToSDJSRect(frame, resultObject.coordScaleFactor);
    objectConfig.r = ShapeDataUtil.ToSDJSRect(rect, resultObject.coordScaleFactor);

    // Store original rectangle data
    objectConfig.originalr = ShapeDataUtil.ToSDJSRect(rect, resultObject.coordScaleFactor);
    objectConfig.originalframe = ShapeDataUtil.ToSDJSRect(frame, resultObject.coordScaleFactor);

    objectConfig.inside = ShapeDataUtil.ToSDJSRect(insideRect, resultObject.coordScaleFactor);
    ...
    if (ShapeDataUtil.UnsupportedTypes.indexOf(sourceData.objecttype) >= 0) {
      resultObject.error = ShapeDataUtil.Errors.UnsupportedPanel;
    ...
    switch (sourceData.otype) {
      case ShapeConstant.ObjectTypes.SED_Shape:
        ...
        initialBounds = sourceData.hgframe ?
          ...
          ShapeDataUtil.ToSDJSRect(frame, resultObject.coordScaleFactor);
        ...
        if (isGroup) {
          ...
          shapeInstance = ShapeDataUtil.CreateShapeObject(objectConfig, sourceData, resultObject, sourceData.extraflags);
        }
      ...
      case ShapeConstant.ObjectTypes.SED_Array:
        ...
        objectConfig.fixedpoint = sourceData.lfixedpoint ?
          ...
          ShapeDataUtil.ToSDJSCoords(sourceData.fixedpoint, resultObject.coordScaleFactor);
    ...
    if (shapeInstance) {
      ...
      ShapeDataUtil.DefaultObject(sessionObject, shapeInstance);
      ...
      if (resultObject.PVersion < ShapeDataUtil.SDF_PVERSION849) {
      ...
      shapeInstance.sizedim.width = ShapeDataUtil.ToSDJSCoords(sizeDimensions.x, resultObject.coordScaleFactor);
      shapeInstance.sizedim.height = ShapeDataUtil.ToSDJSCoords(sizeDimensions.y, resultObject.coordScaleFactor);
      ...
      if (applyStyles) {
        ...
        if (shapeInstance.objecttype) {
          switch (shapeInstance.objecttype) {
            ...
            case ConstantData.ObjectTypes.SD_OBJT_SWIMLANE_COLS:
              ...
              if (resultObject.PVersion < ShapeDataUtil.SDF_PVERSION864) {
        ...
        if (sourceData.colorchanges !== undefined) {
          ...
          ShapeDataUtil.BuildColorChanges(shapeInstance, resultObject);
        }
        ...
        shapeInstance.dimensionDeflectionH = ShapeDataUtil.ToSDJSCoords(sourceData.dimensionDeflectionH, resultObject.coordScaleFactor);
        shapeInstance.dimensionDeflectionV = ShapeDataUtil.ToSDJSCoords(sourceData.dimensionDeflectionV, resultObject.coordScaleFactor);

        // Set hook displacement
        if (sourceData.hookdisp_x || sourceData.hookdisp_y) {
          shapeInstance.hookdisp.x = ShapeDataUtil.ToSDJSCoords(sourceData.hookdisp_x, resultObject.coordScaleFactor);
          shapeInstance.hookdisp.y = ShapeDataUtil.ToSDJSCoords(sourceData.hookdisp_y, resultObject.coordScaleFactor);
        } else {
  ...
  static WriteSelect(selectedObjects, skipTables, unused, preserveSegmentDirection, ignoreDataCheck) {
    ...
    return ShapeDataUtil.WriteBuffer(result, true, true, ignoreDataCheck);
  }
  ...
  static WriteBuffer(resultObject, isSelectOnly, returnRawBuffer, ignoreDataCheck) {
    ...
    dataStream.endianness = T3DataStream.LITTLE_ENDIAN;
    dataStream.writeCString(ShapeDataUtil.Signature, ShapeDataUtil.Signature.length);

    // Handle special file format versions and set coordinate scale factor
    if (resultObject.WriteWin32) {
      ShapeDataUtil.WriteCVersion(dataStream, ShapeConstant.Platforms.SDF_SDJS, ShapeDataUtil.FVERSION2015);
      resultObject.coordScaleFactor = ShapeDataUtil.DRAWRES / T3Gv.docUtil.svgDoc.docInfo.docDpi;
    } else {
      ShapeDataUtil.WriteCVersion(
        dataStream,
    ...
    if (isSelectOnly) {
      ShapeDataUtil.WriteSelectHeader(dataStream, resultObject);
      if (resultObject.error) return null;
    } else {
      ShapeDataUtil.WriteHeader(dataStream, resultObject, null);
      if (resultObject.error) return null;
    }
    ...
    ShapeDataUtil.WriteDraw(dataStream, resultObject);
  ...
  static WriteCVersion(dataStream, platform, fileVersion) {
    ...
    const versionInfo = {
      ...
      PVersion: ShapeDataUtil.SDF_PVERSION,
      Platform: platform,
      MinVer: fileVersion,
      printres: ShapeDataUtil.PRINTRES,
      drawres: ShapeDataUtil.DRAWRES,
      LongFormat: 1,
      TrialVersion: 0,
      Unicode: 1
    };
    ...
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cVersion);

    // Write the version structure and its length
    dataStream.writeStruct(ShapeConstant.VersionStruct, versionInfo);
    ShapeDataUtil.WriteLength(dataStream, codeOffset);
  }
  ...
  static WriteHeader(dataStream, resultObject, skipCodes) {
    ...
    ShapeDataUtil.WriteCHeader(dataStream, resultObject);
    ShapeDataUtil.WriteCPage(dataStream, resultObject);
    ...
    if (skipCodes == null) {
      ...
      ShapeDataUtil.WriteString(dataStream, exportPath, opCodes.cExportPath, resultObject);
    }
  ...
  static WriteString(dataStream, stringValue, opCode, resultObject) {
    if (stringValue != null && stringValue.length) {
      const codeOffset = ShapeDataUtil.WriteCode(dataStream, opCode);
  ...
  static WriteCHeader(dataStream, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cHeader);
  ...
  static WriteCPage(dataStream, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cPage);
    ...
    const fullPageData = {
      ...
      margins: {
        left: ShapeDataUtil.ToSDWinCoords(resultObject.ctp.Page.margins.left, resultObject.coordScaleFactor),
        right: ShapeDataUtil.ToSDWinCoords(resultObject.ctp.Page.margins.right, resultObject.coordScaleFactor),
        top: ShapeDataUtil.ToSDWinCoords(resultObject.ctp.Page.margins.top, resultObject.coordScaleFactor),
        bottom: ShapeDataUtil.ToSDWinCoords(resultObject.ctp.Page.margins.bottom, resultObject.coordScaleFactor)
      },
      minmarg: {
      ...
      lPadDim: {
        x: ShapeDataUtil.ToSDWinCoords(resultObject.sdp.dim.x, resultObject.coordScaleFactor),
        y: ShapeDataUtil.ToSDWinCoords(resultObject.sdp.dim.y, resultObject.coordScaleFactor)
      },
      lpapersize: {
        x: ShapeDataUtil.ToSDWinCoords(resultObject.ctp.Page.papersize.x, resultObject.coordScaleFactor),
        y: ShapeDataUtil.ToSDWinCoords(resultObject.ctp.Page.papersize.y, resultObject.coordScaleFactor)
      },
      MinSize: {
        x: ShapeDataUtil.ToSDWinCoords(minSizeDimensions.x, resultObject.coordScaleFactor),
        y: ShapeDataUtil.ToSDWinCoords(minSizeDimensions.y, resultObject.coordScaleFactor)
      },
  ...
  static WriteUIInfo(dataStream, resultObject) {
    let codeOffset;
    if (dataStream) {
      codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cHeadUiInfo);
    }
  ...
  static WriteNativeID(dataStream, nativeId, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cNativeId);
    const nativeIdData = {
      value: nativeId
    };
  ...
  static WriteTableID(dataStream, tableId, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cTableId);
    const tableIdData = {
      value: tableId
    };
  ...
  static WriteLongValue(dataStream, operationCode, longValue, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, operationCode);
    const longValueData = {
      value: longValue
    };
  ...
  static WriteGraphID(dataStream, graphId, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cGraphId);
    const graphIdData = {
      value: graphId
    };
  ...
  static WriteExpandedViewID(dataStream, expandedViewId, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cExpandedViewId);
  ...
  static WriteGanttInfoID(dataStream, ganttInfoId, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cGanttInfoId);
    const ganttInfoData = {
      value: ganttInfoId
    };
  ...
  static WriteCellNoteID(dataStream, cellNoteId, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cNoteId);
    const cellNoteData = {
      value: cellNoteId
    };
  ...
  static WriteBlobBytesID(dataStream, blobBytesId, blobType, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cImageId);
    const blobBytesData = {
      value: blobBytesId,
      type: blobType
    };
  ...
  static WriteEMFBlobBytesID(dataStream, emfBlobBytesId, emfBlobType, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cEmfId);
    const emfBlobData = {
  ...
  static WriteOleBlobBytesID(dataStream, oleBlobBytesId, oleBlobType, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cOleStorageId);
    const oleBlobData = {
  ...
  static WriteTextureList(dataStream, textureContainer, resultObject) {
    ...
    const listCodeOffset = ShapeDataUtil.WriteCode(dataStream, opCodes.oTextureList);
    dataStream.writeUint32(0);
    ShapeDataUtil.WriteLength(dataStream, listCodeOffset);
    ...
    for (let textureIndex = 0; textureIndex < textureCount; textureIndex++) {
      ...
      const textureData = {
        dim: {
          x: ShapeDataUtil.ToSDWinCoords(texture.dim.x, resultObject.coordScaleFactor),
          y: ShapeDataUtil.ToSDWinCoords(texture.dim.y, resultObject.coordScaleFactor)
        },
        mr: {
      ...
      let codeOffset = ShapeDataUtil.WriteCode(dataStream, opCodes.oTexture);
      ...
      codeOffset = ShapeDataUtil.WriteCode(dataStream, opCodes.oTextureExtra);
      dataStream.writeStruct(ShapeConstant.TextureExtraStruct, textureScaleData);
      ShapeDataUtil.WriteLength(dataStream, codeOffset);

      // Write texture name
      ShapeDataUtil.WriteString(dataStream, texture.name, opCodes.oTextureName, resultObject);

      // Write binary data for custom textures
      if (!(texture.flags & ShapeConstant.TextureFlags.SD_Tx_Std) && texture.BlobBytes) {
        ShapeDataUtil.WriteBlob(dataStream, texture.BlobBytes, opCodes.oTextureData);
      }
    }
  ...
  static WriteDraw(dataStream, resultObject) {
    ...
    ShapeDataUtil.BuildStyleList(resultObject);

    // Get session data reference
    sessionData = resultObject.sdp;

    // Write primary drawing structure and elements
    ShapeDataUtil.WriteCDraw12(dataStream, resultObject);
    ShapeDataUtil.WriteStyle(dataStream, sessionData.def.style, true, resultObject, null);
    ShapeDataUtil.WriteSDLine(
      dataStream,
    ...
    ShapeDataUtil.WriteRulers(dataStream, resultObject);
    ShapeDataUtil.WriteRecentList(dataStream, resultObject);
    ShapeDataUtil.WriteLayers(dataStream, resultObject);
    ShapeDataUtil.WriteLinks(dataStream, resultObject);
    ShapeDataUtil.WriteTextureList(dataStream, T3Gv.opt.TextureList, resultObject);
    ShapeDataUtil.WriteStyleList(dataStream, resultObject.lpStyles, false, resultObject);
    ...
    for (uniqueMapIndex = 0; uniqueMapIndex < uniqueMapLength; uniqueMapIndex++) {
      ...
      if (objectId < 0) {
        ...
        if (lastConnectorIndex >= 0) {
          ...
          textAlign = ShapeDataUtil.TextAlignToWin(connectorObject.TextAlign);
          hookData = ShapeDataUtil.SetHookByJust(textAlign.just, textAlign.vjust, rectObject.hooks[0].connect);
          ...
          if (currentObject && currentObject.DrawingObjectBaseClass === ConstantData.DrawingObjectBaseClass.LINE) {
            // Handle line text
            textAlign = ShapeDataUtil.TextAlignToWin(currentObject.TextAlign);
            hookData = ShapeDataUtil.SetHookByJust(textAlign.just, textAlign.vjust, rectObject.hooks[0].connect);
            rectObject.hooks[0].hookpt = hookData.hookpt;
      ...
      if (currentObject) {
        ShapeDataUtil.WriteObject(dataStream, uniqueMapIndex, currentObject, resultObject);
      }
    }
  ...
  static WriteRulers(dataStream, resultObject) {
    const rulerData = {
      ...
      Major: ShapeDataUtil.ToSDWinCoords(resultObject.rulerConfig.major, resultObject.coordScaleFactor),
    ...
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.oRuler);
  ...
  static WriteRecentList(dataStream, resultObject) {
    ...
    if (
      ...
      const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cRecentSymbolsBegin);
      ShapeDataUtil.WriteLength(dataStream, codeOffset);
  ...
  static WriteLayers(dataStream, resultObject) {
    ...
    const beginCodeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cBeginLayer);
    ShapeDataUtil.WriteLength(dataStream, beginCodeOffset);
    ...
    for (let layerIndex = 0; layerIndex < layerCount; ++layerIndex) {
      ...
      // Write layer flags
      const flagsCodeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cLayerFlags);
      dataStream.writeUint32(currentLayer.flags);
      ShapeDataUtil.WriteLength(dataStream, flagsCodeOffset);
      ...
      // Write layer type
      const typeCodeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cLayerType);
      dataStream.writeUint32(currentLayer.layertype);
      ShapeDataUtil.WriteLength(dataStream, typeCodeOffset);
      ...
      if (resultObject.WriteBlocks) {
        ...
        const listCodeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cLayerList);
  ...
  static WriteLinks(dataStream, resultObject) {
    ...
    if (standardLinkCount || textLinkCount || resultObject.WriteBlocks) {
      ...
      for (let linkIndex = 0; linkIndex < standardLinkCount; linkIndex++) {
        ...
        linkData = {
          targetid: ShapeDataUtil.BlockIDtoUniqueID(standardLinks[linkIndex].targetid, resultObject),
          tindex: -1,
          hookid: ShapeDataUtil.BlockIDtoUniqueID(standardLinks[linkIndex].hookid, resultObject),
          hindex: -1,
      ...
      const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cDrawLink);
  ...
  static WriteCDraw12(dataStream, resultObject) {
    ...
    const selectedObjectId = sessionData.tselect >= 0
      ? (resultObject.WriteBlocks ? sessionData.tselect : ShapeDataUtil.BlockIDtoUniqueID(sessionData.tselect, resultObject))
      : -1;
    ...
    const logicalFont = ShapeDataUtil.FontRecToLogFont(fontList[0], 0, resultObject);
    ...
    const drawingData = {
      ...
      unique: objectCount,
      dupdisp: {
        x: ShapeDataUtil.ToSDWinCoords(resultObject.sdp.dupdisp.x, resultObject.coordScaleFactor),
        y: ShapeDataUtil.ToSDWinCoords(resultObject.sdp.dupdisp.y, resultObject.coordScaleFactor)
      },
      just: ShapeDataUtil.JSJustToWin(sessionData.def.just),
      vjust: ShapeDataUtil.JSJustToWin(sessionData.def.vjust),
      d_sarrow: startArrow,
      d_earrow: endArrow,
      ...
      hopdim: {
        x: ShapeDataUtil.ToSDWinCoords(resultObject.sdp.hopdim.x, resultObject.coordScaleFactor),
        y: ShapeDataUtil.ToSDWinCoords(resultObject.sdp.hopdim.y, resultObject.coordScaleFactor)
      },
      ...
      tmargins: {
        left: ShapeDataUtil.ToSDWinCoords(sessionData.def.tmargins.left, resultObject.coordScaleFactor),
        right: ShapeDataUtil.ToSDWinCoords(sessionData.def.tmargins.right, resultObject.coordScaleFactor),
        top: ShapeDataUtil.ToSDWinCoords(sessionData.def.tmargins.top, resultObject.coordScaleFactor),
        bottom: ShapeDataUtil.ToSDWinCoords(sessionData.def.tmargins.bottom, resultObject.coordScaleFactor)
      },
      textgrow: sessionData.def.textgrow,
      textflags: sessionData.def.textflags,
      fsize_min: sessionData.def.fsize_min,
      styleindex: -1,
      h_arraywidth: ShapeDataUtil.ToSDWinCoords(sessionData.def.h_arraywidth, resultObject.coordScaleFactor),
      v_arraywidth: ShapeDataUtil.ToSDWinCoords(sessionData.def.v_arraywidth, resultObject.coordScaleFactor),
      ...
      arraywd: ShapeDataUtil.ToSDWinCoords(sessionData.def.arraywd, resultObject.coordScaleFactor),
      arrayht: ShapeDataUtil.ToSDWinCoords(sessionData.def.arrayht, resultObject.coordScaleFactor),
    ...
    // Write appropriate structure based on format
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cDraw12);
    if (resultObject.WriteWin32) {
  ...
  static WriteStyle(dataStream, styleRecord, useBorder, resultObject, styleObject) {
    ...
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cBeginStyle);
    ...
    if (styleRecord.Fill) {
      ShapeDataUtil.WriteSDFill(dataStream, styleRecord.Fill, resultObject);
    }
    ...
    ShapeDataUtil.WriteSDTxf(dataStream, styleRecord.Text, resultObject);
  ...
  static WriteStyleList(dataStream, styleArray, useBorder, resultObject) {
    ...
    ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cBeginStyleList);

    // Write each style in the array
    styleCount = styleArray.length;
    for (styleIndex = 0; styleIndex < styleCount; styleIndex++) {
      ShapeDataUtil.WriteStyle(dataStream, styleArray[styleIndex], useBorder, resultObject, null);
    }
  ...
  static WriteSDFill(dataStream, fillData, resultObject) {
    ...
    ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cBeginFill);
    ...
    if (fillData.Hatch) {
      const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cHatch);
    ...
    if (fillData.FillEffect) {
      ShapeDataUtil.WriteEffect(dataStream, fillData, fillData.FillEffect);
    }
  ...
  static WriteSDLine(dataStream, lineData, resultObject, opCode, styleObject) {
    ...
    lineThickness = ShapeDataUtil.ToSDWinCoords(lineData.Thickness, resultObject.coordScaleFactor);
    ...
    ShapeDataUtil.WritePaint(dataStream, lineData.Paint, ConstantData.Colors.Color_Black, resultObject);
    ...
    if (fillLineThickness) {
      ...
      const filledLineOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cFilledLine);
      dataStream.writeStruct(ShapeConstant.FilledLineStruct, filledLineData);
      ShapeDataUtil.WriteLength(dataStream, filledLineOffset);
    }

    // Write hatch pattern if present
    if (lineData.Hatch) {
      const hatchOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cHatch);
      dataStream.writeUint32(lineData.Hatch);
      ShapeDataUtil.WriteLength(dataStream, hatchOffset);
    }

    // Write line effect if present
    if (lineData.LineEffect) {
      ShapeDataUtil.WriteEffect(dataStream, lineData, lineData.LineEffect);
    }
  ...
  static WriteSDTxf(dataStream, textFormatting, resultObject) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cBeginTextf);
    ...
    ShapeDataUtil.WritePaint(dataStream, textFormatting.Paint, ConstantData.Colors.Color_Black, resultObject);

    // Write text effects if present
    if (textFormatting.Effect.OutsideType) {
      ShapeDataUtil.WriteOutside(dataStream, textFormatting.Effect);
    }
  ...
  static WriteOutside(dataStream, outsideEffect) {
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cOutSide);
    ...
    const colorValue = outsideEffect.Color
      ? ShapeDataUtil.HTMLColorToWin(outsideEffect.Color)
      : ShapeDataUtil.HTMLColorToWin(ConstantData.Colors.Color_Black);
  ...
  static WritePaint(dataStream, paintData, defaultColor, resultObject) {
    ...
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cBeginPaint);
    ...
    const paintStruct = {
      ...
      color: ShapeDataUtil.HTMLColorToWin(paintData.Color, paintData.Opacity)
    };
    ...
    switch (paintData.FillType) {
      case ConstantData.FillTypes.SDFILL_GRADIENT:
        ...
        const gradientData = {
          ecolor: ShapeDataUtil.HTMLColorToWin(paintData.EndColor, paintData.EndOpacity),
          gradientflags: paintData.GradientFlags
        };

        const gradientOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cGradient);
        dataStream.writeStruct(ShapeConstant.GradientStruct, gradientData);
        ShapeDataUtil.WriteLength(dataStream, gradientOffset);
        break;
      ...
      case ConstantData.FillTypes.SDFILL_RICHGRADIENT:
        ...
        if (richGradient) {
          ...
          const richGradientOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cRichGradient);
          dataStream.writeStruct(ShapeConstant.RichGradientStruct, richGradientData);
          ShapeDataUtil.WriteLength(dataStream, richGradientOffset);

          // Write each gradient stop
          for (let stopIndex = 0; stopIndex < stopCount; stopIndex++) {
            const stopData = {
              color: ShapeDataUtil.HTMLColorToWin(richGradient.stops[stopIndex].color, richGradient.stops[stopIndex].opacity),
              stop: richGradient.stops[stopIndex].stop
            };

            const stopOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cRichGradientStop);
            dataStream.writeStruct(ShapeConstant.RichGradientStopStruct, stopData);
            ShapeDataUtil.WriteLength(dataStream, stopOffset);
          }
        }
        break;
      ...
      case ConstantData.FillTypes.SDFILL_TEXTURE:
        // Write texture fill
        const textureOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cTexture);
        ...
        ShapeDataUtil.WriteLength(dataStream, textureOffset);
        break;
    }
  ...
  static ShapeParamToSDR(shapeData, resultObject) {
    ...
    switch (shapeData.dataclass) {
      ...
      case shapeTypes.HEXAGON:
        convertedParam = ShapeDataUtil.ToSDWinCoords(shapeData.shapeparam, resultObject.coordScaleFactor);
        break;
  ...
  static WriteObject(e, t, a, r) {
    var i,
      ...
      y = ShapeDataUtil.WriteCode(e, ShapeConstant.OpNameCode.cDrawObj8),
      f = - 1,
    ...
    u = a,
    a.associd > 0 &&
    (f = ShapeDataUtil.BlockIDtoUniqueID(a.associd, r)),
    ...
    S
          ...
          case ConstantData.LineType.LINE:
            a.LineType == ConstantData.LineType.ARCLINE &&
              (
                ...
                n = ShapeDataUtil.ToSDWinCoords(a.CurveAdjust, r.coordScaleFactor),
                a.IsReversed &&
                (n = - n),
                (
              ...
              Math.abs(a.StartPoint.x - a.EndPoint.x) < ConstantData.Defines.MinLineDistanceForDeterminingOrientation ? (
                d = ShapeDataUtil.ToSDWinCoords(a.StartPoint.x + r.GroupOffset.x, r.coordScaleFactor),
                ...
                d = ShapeDataUtil.ToSDWinCoords(a.StartPoint.y + r.GroupOffset.y, r.coordScaleFactor),
      ...
      case ConstantData.DrawingObjectBaseClass.SHAPE:
        ...
        C = ShapeDataUtil.ToSDWinRect(m, r.coordScaleFactor, {
          x: 0,
          y: 0
        }),
        ...
        (
          m.x = a.Frame.x,
          m.y = a.Frame.y,
          C = ShapeDataUtil.ToSDWinRect(m, r.coordScaleFactor, r.GroupOffset)
        ),
        ...
        c
          ...
          case ConstantData.ShapeType.POLYGON:
            a.dataclass ? (D = a.dataclass, n = ShapeDataUtil.ShapeParamToSDR(a, r)) : (
    ...
    var _ = {
      otype: g,
      r: ShapeDataUtil.ToSDWinRect(u.r, r.coordScaleFactor, r.GroupOffset),
      frame: ShapeDataUtil.ToSDWinRect(s, r.coordScaleFactor, r.GroupOffset),
      inside: ShapeDataUtil.ToSDWinRect(u.inside, r.coordScaleFactor, r.GroupOffset),
      dataclass: D,
      flags: o,
      extraflags: A,
      fixedpoint: d,
      ...
      objgrow: a.ObjGrow,
      sizedim: {
        x: ShapeDataUtil.ToSDWinCoords(a.sizedim.width, r.coordScaleFactor),
        y: ShapeDataUtil.ToSDWinCoords(a.sizedim.height, r.coordScaleFactor)
      },
      ...
      hiliter: ShapeDataUtil.ToSDWinRect(a.Frame, r.coordScaleFactor, r.GroupOffset),
    ...
    if (
      ...
      ShapeDataUtil.WriteLength(e, y),
      ShapeDataUtil.WriteHooks(e, a, r),
      ShapeDataUtil.WriteObjData(e, a, r),
      a.HyperlinkText &&
      ShapeDataUtil.WriteString(e, a.HyperlinkText, L.cDrawJump, r),
      r.WriteBlocks ||
      r.WriteGroupBlock ||
      ShapeDataUtil.WriteNotes(e, a, r),
      a.flags & ConstantData.ObjFlags.SEDO_UseConnect &&
      ShapeDataUtil.WriteConnectPoints(e, a),
      a.StyleRecord
    ) {
      ...
      r.WriteVisio &&
        ...
        ShapeDataUtil.WriteStyleOverrides(e, a, r),
        a.StyleRecord.Fill.Paint.FillType = E
    }
    a.BusinessName &&
      ShapeDataUtil.WriteString(e, a.BusinessName, L.cBusinessNameStr, r),
  ...
  static WriteNotes(dataStream, drawingObject, resultObject) {
    if (drawingObject.NoteID !== -1) {
      ShapeDataUtil.WriteText(dataStream, drawingObject, null, null, true, resultObject);
    }
  }
  ...
  static WriteHooks(dataStream, drawingObject, resultObject) {
    ...
    if (drawingObject.DrawingObjectBaseClass === ConstantData.DrawingObjectBaseClass.LINE) {
      isLineReversed = ShapeDataUtil.LineIsReversed(drawingObject, resultObject, false);
    ...
    if (hookCount) {
      for (hookIndex = 0; hookIndex < hookCount; hookIndex++) {
        ...
        if (ShapeDataUtil.LineIsReversed(targetObject, resultObject, true)) {
        ...
        codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cDrawHook);
        hookData = {
  ...
  static WriteObjData(dataStream, drawingObject, resultObject) {
    ...
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cObjData);
  ...
  static WriteStyleOverrides(dataStream, drawingObject, resultObject) {
    ...
    if (drawingObject.tstyleindex >= 0 &&
      ...
      //   ShapeDataUtil.WriteSDFill(dataStream, drawingObject.StyleRecord.Fill, resultObject);
      // }
      ...
      //   ShapeDataUtil.WriteOutside(dataStream, drawingObject.StyleRecord.OutsideEffect);
      // }

    }
    // If no valid style reference exists, write the complete style
    else {
      ShapeDataUtil.WriteStyle(dataStream, drawingObject.StyleRecord, false, resultObject, drawingObject);
    }
  }
  ...
  static WriteArrowheads(dataStream, resultObject, drawingObject) {
    ...
    if (ShapeDataUtil.LineIsReversed(drawingObject, resultObject, false)) {
    ...
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cDrawArrow);
  ...
  static GetBlockName(storedObject, skipSystemObjects, blockMetadata) {
    ...
    switch (storedObject.Type) {
      case objectTypes.BASE_LM_DRAWING_OBJECT:
        blockName = ShapeDataUtil.BlockNames.LMObject + storedObject.ID;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.LMObject;
        }
        break;

      case objectTypes.LM_TEXT_OBJECT:
      case objectTypes.LM_NOTES_OBJECT:
        blockName = ShapeDataUtil.BlockNames.Text + storedObject.ID;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.Text;
        }
        break;

      case objectTypes.TABLE_OBJECT:
        blockName = ShapeDataUtil.BlockNames.Table + storedObject.ID;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.Table;
        }
        break;

      case objectTypes.GRAPH_OBJECT:
        blockName = ShapeDataUtil.BlockNames.Graph + storedObject.ID;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.Graph;
        }
        break;

      case objectTypes.EXPANDEDVIEW_OBJECT:
        blockName = ShapeDataUtil.BlockNames.ExpandedView + storedObject.ID;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.ExpandedView;
        }
        break;

      case objectTypes.LM_COMMENT_BLOCK:
        blockName = ShapeDataUtil.BlockNames.Comment + storedObject.ID;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.Comment;
        }
        break;

      case objectTypes.H_NATIVE_OBJECT:
      case objectTypes.H_NATIVEWIN_OBJECT:
        blockName = ShapeDataUtil.BlockNames.Native + storedObject.ID;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.Native;
        }
        break;

      case objectTypes.BLOBBYTES_OBJECT:
        blockName = ShapeDataUtil.BlockNames.Image + storedObject.ID;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.Image;
        }
        break;

      case objectTypes.SED_SESSION_OBJECT:
        if (skipSystemObjects) return null;
        blockName = ShapeDataUtil.BlockNames.sdp;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.sdp;
        }
        break;

      case objectTypes.LAYERS_MANAGER_OBJECT:
        if (skipSystemObjects) return null;
        blockName = ShapeDataUtil.BlockNames.Layers;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.Layers;
        }
        break;

      case objectTypes.SDDATA_OBJECT:
        if (skipSystemObjects) return null;
        blockName = ShapeDataUtil.BlockNames.SDData;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.SDData;
        }
        break;

      case objectTypes.LINKLIST_OBJECT:
        if (skipSystemObjects) return null;
        blockName = ShapeDataUtil.BlockNames.Links;
        if (blockMetadata) {
          blockMetadata.id = storedObject.ID;
          blockMetadata.type = ShapeDataUtil.BlockIDs.Links;
        }
    }
    ...
    switch (storedObject.Type) {
      case objectTypes.BASE_LM_DRAWING_OBJECT:
        ...
        if (objectInstance && !objectInstance.Data.bInGroup) {
          ...
          serializedBlock = ShapeDataUtil.WriteOBJBlock(objectInstance.Data, resultObject, blockIndex);
        }
        break;
      ...
      case objectTypes.LM_TEXT_OBJECT:
        ...
        if (objectInstance) {
          if (countOnly) return true;
          serializedBlock = ShapeDataUtil.WriteTextBlock(objectInstance, resultObject, false, blockIndex);
        }
        break;
      ...
      case objectTypes.LM_NOTES_OBJECT:
        ...
        if (objectInstance) {
          if (countOnly) return true;
          serializedBlock = ShapeDataUtil.WriteTextBlock(objectInstance, resultObject, true, blockIndex);
        }
        break;
      ...
      case objectTypes.TABLE_OBJECT:
        ...
        if (objectInstance) {
          if (countOnly) return true;
          serializedBlock = ShapeDataUtil.WriteTableBlock(objectInstance, resultObject, blockIndex);
        }
        break;
      ...
      case objectTypes.GRAPH_OBJECT:
        ...
        if (objectInstance) {
          if (countOnly) return true;
          serializedBlock = ShapeDataUtil.WriteGraphBlock(objectInstance, resultObject, blockIndex);
        }
        break;
      ...
      case objectTypes.EXPANDEDVIEW_OBJECT:
        ...
        if (objectInstance) {
          if (countOnly) return true;
          serializedBlock = ShapeDataUtil.WriteExpandedViewBlock(objectInstance, resultObject, blockIndex);
        }
        break;
      ...
      case objectTypes.LM_COMMENT_BLOCK:
        ...
        if (objectInstance) {
          if (countOnly) return true;
          serializedBlock = ShapeDataUtil.WriteCommentBlock(objectInstance, resultObject, blockIndex);
        }
        break;
      ...
      case objectTypes.BLOBBYTES_OBJECT:
        ...
        if (objectInstance) {
          if (countOnly) return true;
          serializedBlock = ShapeDataUtil.WriteImageBlock(objectInstance, resultObject, blockIndex);
        }
        break;
      ...
      case objectTypes.SED_SESSION_OBJECT:
        ...
        if (countOnly) return true;
        serializedBlock = ShapeDataUtil.WriteSDPBlock(resultObject, blockIndex);
        break;
      ...
      case objectTypes.LAYERS_MANAGER_OBJECT:
        ...
        if (countOnly) return true;
        serializedBlock = ShapeDataUtil.WriteLayersBlock(resultObject.tLMB, resultObject, blockIndex);
        break;
      ...
      case objectTypes.LINKLIST_OBJECT:
        ...
        if (objectInstance) {
          if (countOnly) return true;
          serializedBlock = ShapeDataUtil.WriteLinksBlock(objectInstance.Data, resultObject, blockIndex);
        }
        break;
    }
  ...
  static SaveChangedBlocks(stateId, deltaState, targetStateId, customStoredObjects) {
    try {
      ...
      if (deltaState < 0 && stateId + 1 < T3Gv.state.States.length) {
        ...
        for (let objIndex = 0; objIndex < nextStateObjectCount; objIndex++) {
          ...
          if (currentObject.StateOperationTypeID === Globals.StateOperationType.CREATE) {
            if (ShapeDataUtil.GetBlockName(currentObject, true)) {
              blockCount++;
            }
          }
          // Handle deleted objects in the next state
            ...
            if (ShapeDataUtil.GetBlockName(currentObject, true)) {
              if (ShapeDataUtil.BuildObjectBlock(currentObject, resultObject, true, 0)) {
                blockCount++;
      ...
      resultObject.BlockAction = ShapeDataUtil.BlockActions.Normal;
      ...
      // Write header block
      serializedBlock = ShapeDataUtil.WriteHeaderBlock(resultObject, blockIndex, ShapeDataUtil.HeaderFilters);
      blockIndex++;
      ...
      if (deltaState < 0 && stateId + 1 < T3Gv.state.States.length) {
        ...
        for (let objIndex = 0; objIndex < nextStateObjectCount; objIndex++) {
          ...
          if (currentObject.StateOperationTypeID === Globals.StateOperationType.CREATE) {
            if (ShapeDataUtil.GetBlockName(currentObject, true, blockMetadata)) {
              serializedBlock = ShapeDataUtil.WriteActionBlock(
                resultObject,
                blockMetadata.type,
                blockMetadata.id,
                ShapeDataUtil.BlockActions.Delete,
                blockIndex
              );
              blockIndex++;
            }
          ...
          // Handle deleted objects in next state
            ...
            serializedBlock = ShapeDataUtil.BuildObjectBlock(objectInstance, resultObject, false, blockIndex);
      ...
      for (let objIndex = 0; objIndex < processCount; objIndex++) {
        ...
        // Handle other objects not already processed
          ...
          serializedBlock = ShapeDataUtil.BuildObjectBlock(currentObject, resultObject, false, blockIndex);
          // Increment handled in BuildObjectBlock
        }
      }
    } catch (error) {
  ...
  static WriteActionBlock(resultObject, blockType, blockId, actionType, blockIndex) {
    ...
    ShapeDataUtil.WriteBlockWrapper(
      dataStream,
      resultObject.state,
      resultObject.delta,
      blockType,
  ...
  static WriteSDPBlock(resultObject, blockIndex) {
    ...
    ShapeDataUtil.WriteStyle(dataStream, resultObject.sdp.def.style, true, resultObject, null);
    ...
    if (resultObject.sdp.background.Paint.FillType !== ConstantData.FillTypes.SDFILL_TRANSPARENT) {
      ShapeDataUtil.WriteSDFill(dataStream, resultObject.sdp.background, resultObject);
    }
  ...
  static WriteOBJBlock(drawingObject, resultObject, blockIndex) {
    ...
    ShapeDataUtil.WriteBlockWrapper(
      ...
      resultObject.state,
      resultObject.delta,
      ShapeDataUtil.BlockIDs.LMObject,
    ...
    ShapeDataUtil.WriteObject(dataStream, 0, drawingObject, resultObject);
  ...
  static WriteHeaderBlock(resultObject, blockIndex, headerFilters) {
    ...
    if (headerFilters == null) {
      ...
      ShapeDataUtil.WriteBlockWrapper(
        ...
        resultObject.state,
        resultObject.delta,
        ShapeDataUtil.BlockIDs.Header2,
        ShapeDataUtil.Header2Count++,
        blockIndex,
        resultObject.nblocks,
        resultObject.BlockAction
      );
    ...
    ShapeDataUtil.WriteHeader(dataStream, resultObject, headerFilters);
  ...
  static WriteBlob(dataStream, binaryData, operationCode) {
    ...
    const lengthPosition = ShapeDataUtil.WriteCode(dataStream, operationCode);
  ...
  static WriteImageHeader(dataStream, drawingObject, resultObject) {
    ...
    if (drawingObject.ImageHeader) {
      ...
      measurementRect = ShapeDataUtil.ToSDWinRect(drawingObject.Frame, resultObject.coordScaleFactor, null);
      cropRect = {
        left: 0,
        top: 0,
        right: 0,
      ...
      scaleValue = 1;
      imageFlags = ConstantData.ImageScales.SDIMAGE_ALWAYS_FIT;
    }
    ...
    const codeOffset = ShapeDataUtil.WriteCode(dataStream, ShapeConstant.OpNameCode.cDrawImage8);
